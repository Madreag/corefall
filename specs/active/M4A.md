...
M4A is **the closure milestone for DR-003 (body damage readability) AND DR-012 (accessibility floor)**. Every UI surface from M4A forward inherits the ACC-A floor automatically because the `Settings` resource + `HudSettings` mirror + Bevy `UiScale` + palette swap + caption strip + `observe.accessibility` are workspace-wide.
...
| **L2 — Squad / tactical state** | 2-5 s | Which bot needs help, path blocked, current doctrine, LZ risk, material overlay legend, mission timer, objective sub-line, reactor pressure (M2.5) | Squad strip, command overlay, material overlay, quick-detail drawer |
| **L3 — Planning / economy** | 5-60 s | Buy/loadout panel, role fit, delivery craft choice, package/server compatibility, replay browser rows | Full panel/table with filters, sort, comparison (M8 forward-compat) |
| **L4 — Debug / learning** | After-action | Death recap cause chain, AI failure tags, package diagnostics, replay event filters, backend health, accessibility audit | Viewer/report/workbench/hub screens (M3B + M8 already / future) |
...
- **Bottom-right (EVENT TICKER, L1-L2)**: last-important-event ticker (rendered via plain-language templates from M3B's renderer; max 1 event visible at a time; auto-dismisses after 4 seconds)
- **Left edge (SILHOUETTE + MODULE STRIP, L1)**: body silhouette (per-zone HP%, head/torso/arms/legs at M4A scalar; chassis-backed per-zone at M5) + module strip (up to 5 modules: WEAPON / JET / SHIELD / SENSOR / REPAIR with state tag OK/DEG/WARN/FAIL/N/A and ammo/power/bound_zone fields)
- **Bottom-center (TOOL + REACTOR, L1-L2)**: TOOL validity line ("DIGGER: VALID" / "DIGGER: REFUSED — cannot dig metal") + REACTOR pressure line (M2.5 forward-compat: "REACTOR: NOMINAL/STRESSED/CRITICAL/VENTING/DESTROYED")
...
- **Death recap modal** on `mission.mission_resolved.result=lost` with "Show me why" CTA → M3B (DR-023)
- **Pause/slowdown overlay** (DR-009 forward-compat — M4A ships the overlay state; M4B + M7+ implement command UX)

### Status banner taxonomy

Banners are the L0-L1 surface for state changes that need player attention NOW. M4A ships the full taxonomy below; producers ladder up at their owning milestones. Every banner has `(severity, glyph, text_template, parent_event_type, dismiss_condition)`.

**Body damage banners (DR-003 closure — producers in M1 + M5):**

| Banner | Severity | Glyph | Trigger event | Dismiss when |
|---|---|---|---|---|
| `HP_LOW` | warning | `[!]` | `actor.actor_status_changed` to UNSTABLE OR HP < 30% | HP > 50% |
| `BLEEDING_LEFT_ARM` | warning | `[!]` | `wound_added` to arm_left with bleed=true (M5+) | bleed cleared / wound healed |
| `LIMB_LOST_LEFT_LEG` | critical | `[!!]` | `attachable_detached` for leg_left (M5+) | actor death (banner persists otherwise) |
| `ARMOR_CRACKED_TORSO` | critical | `[!!]` | armor layer destroyed at torso (M5+) | armor repaired OR actor death |
| `ARMOR_CRACKED_HEAD` | critical | `[!!]` | armor layer destroyed at head (M5+) | repaired OR death |
| `ARMOR_CRACKED_LEFT_ARM` / `RIGHT_ARM` / `LEFT_LEG` / `RIGHT_LEG` | critical | `[!!]` | per-zone armor destroyed (M5+) | repaired |
| `ARMOR_CHUNKED_<zone>` | warning | `[!]` | armor.chunked_off fires for zone | zone re-armored (M5+) |
...
| `STIMMED` | info | `[*]` | affliction.stim_active (M5.7+) | timer expires |
| `BLEEDING_OUT` | critical | `[!!]` | affliction.bleeding_severe (M5.7+) | stabilized via medikit |

**Deep damage banners (M2.5 NEW):**

| Banner | Severity | Glyph | Trigger | Dismiss |
|---|---|---|---|---|
| `INTERNAL_ORGAN_DAMAGED_<organ>` | warning | `[!]` | internal.organ_damaged | repair (medikit M5+) |
...
| `INTERNAL_SHOCK_MODULE_DAMAGED` | warning | `[!]` | internal_shock.module_damaged | repair (M5+) |
...
**Equipment + weapon banners (M1 + M5):**

| Banner | Severity | Glyph | Trigger | Dismiss |
|---|---|---|---|---|
| `WEAPON_JAMMED` | warning | `[!]` | `chassis.weapon_jammed` (M5) OR rifle jam (M1) | `weapon_cleared` |
| `WEAPON_OVERHEAT` | warning | `[!]` | weapon heat > threshold (M5+) | heat dissipated |
| `EMPTY_MAG` | warning | `[!]` | rifle ammo = 0 | reload completed |
| `RELOADING_NN%` | info | `[*]` | reload_started | reload_completed |
| `NO_AMMO_RESERVE` | warning | `[!]` | all magazines depleted (M5+) | resupply / pickup |
| `LOW_AMMO` | warning | `[!]` | magazine < 25% | magazine refilled |
| `ITEM_BROKEN` | warning | `[!]` | held device durability = 0 (M5+) | item replaced |

**Chassis lifecycle banners (M5):**
...
**Mission + reactor banners (M1.5 + M2.5 + M7):**
...
| `RESCUE_WINDOW` | warning | `[!]` | DR-018 rescue window opens (M5+) | rescue / window closes |
| `MISSION_CRITICAL_ACTOR_DOWN` | critical | `[!!]` | mission_critical actor enters DYING | rescued OR mission_resolved |
| `REACTOR_NOMINAL` | info | `[*]` | M2.5 reactor pressure_state=Nominal first set | state advances |
...
**Threat awareness banners (M1.5 + M5.5+):**
...
| `INCOMING_FIRE` | warning | `[!]` | projectile trajectory toward player (M5.5+) | impact OR miss |
| `INCOMING_EXPLOSIVE` | critical | `[!!]` | grenade/rocket tracked toward player (M5.5+) | detonation |
| `DROPSHIP_INCOMING` | warning | `[!]` | dropship vector toward player vicinity (M7+) | dropship landed |
| `FRIENDLY_FIRE_DANGER` | warning | `[!]` | weapon arc covers friendly (M11+ teams) | aim shifts |
| `HAZARD_ON_PATH` | warning | `[!]` | walking toward hazard tile (M2 + M5.7) | redirected |

**Comms banners (M9.5 forward-compat):**

| Banner | Severity | Glyph | Trigger | Dismiss |
|---|---|---|---|---|
| `RADIO_INCOMING` | info | `[*]` | `radio.transmission_received` (M9.5+) | message ends |
| `RADIO_BLOCKED_JAMMED` | warning | `[!]` | `radio.interference_event` | clear signal |
| `VOICE_BLOCKED_VACUUM` | warning | `[!]` | `voice.transmission_blocked` reason=vacuum (M5.9+) | atmosphere returns |

**Accessibility banners (M4A own surface):**
...
DR-003 closes at M4A with **Recommendation E (hybrid default + advanced opt-in)**. The HUD body surface ladders up additively:
...
- HUD renders the 6-zone widget; tint is plain "hp%" scaled, all zones identical until M5

**M4A (this milestone) — full readability surface:**
- HUD silhouette widget renders 6-zone tinting (red gradient for low HP; gray for "destroyed" placeholder)
- **Wound count + trend display** per CCCP `MOSRotating::GibWoundLimit` (M1.5+ reactive guard emits `wound_added` events; M4A renders the count near the silhouette + trend arrow ↑ when increasing)
...
**M5 (concurrent or post-M4A) — chassis-backed real per-zone:**
...
**M5.7 (forward-compat) — afflictions overlay:**
- Per-actor affliction layer per `affliction.*` event family (M5.7+): wetness, burning, corroded, electrified, poisoned, asphyxiating, concussed, drowning, depressurizing, internal_shock, coolant_leaking, oil_leaking, overheating, low_battery, hypoxia
- HUD shows affliction icons + stack count under the silhouette
- M4A reserves the HUD slot; producers ladder up at M5.7
...
When `chassis_attached=false`: pilot silhouette is the only body widget (M4A default).
...
- `observe.chassis.silhouette` returns the chassis-specific zone graph (per-chassis-shape variant; humanoid=15 zones; quadruped=11 zones; tank=8 zones; reserved at M4A, populated at M5)
...
### Chassis HUD by weight class (M5+ forward-compat)

CCCP-family games (and DR-029 chassis identity) support different chassis weight classes. M4A reserves the surface contract so M5+ classes inherit without renaming:
...
M4A's HUD must NOT hardcode "humanoid bipedal" as the only chassis shape. The silhouette renderer reads `chassis.shape_variant` and draws accordingly.

### Chassis fuel + power + heat gauges (M5+ forward-compat)

Chassis-only resources distinct from actor HP. M4A reserves the HUD slot — placed in the AMMO+ITEM zone when chassis is attached:
...
| **REACTOR_PRESSURE** (heavy class only) | enum | Pressure gauge mirroring M2.5 reactor pressure_state | Existing REACTOR_* banner family |
...
Per `cf-actor::Stance` extensions: `Idle, Walking, Running, Crouching, Airborne, KnockedDown, Downed, Dying, Dead, Ejecting, Climbing, Jetting` + (M5) `Piloting, ChassisWalking, ChassisJetting, ChassisCrouched, ChassisProne, Driving, Boarding, Disembarking, Salvaging`. Stance::from_chassis derives chassis-driven stance when chassis_attached=true.

### Camera anchor + brain-actor switching (M5+ + M11+ forward-compat)

Cortex-family games support **camera anchor switching** (CCCP brain-hopping or actor-switching). M4A reserves the surface:

- **First-person mech-cockpit view** (M5+): when piloting a chassis, the camera can dock at the cockpit anchor; HUD adds a "COCKPIT" badge + sharper field-of-view tint
- **Third-person actor view** (default): when out of chassis OR commanding, camera follows the actor at default distance
- **Brain camera (M11+)**: when the player IS the brain (high-value command actor), the brain warning pulses near the silhouette ("BRAIN AT RISK")
- **Switching between controlled actors** (M11+ squad command OR DR-022 humanlike): brief camera transition (200ms ease-out) + caption "Switched to <actor name>" + previous-actor handoff order summary
- **act.input.switch_actor { actor_id }** (forward-compat; M11+): cycles controlled actor; emits `ux.actor_switched` event
- **Brain-threat banner**: critical banner when brain HP < 30% OR enemy LOS to brain (CCCP pattern from `GameActivity.h:204-241`)

### Delivery + dropship + craft surface (M7+ forward-compat)

Cortex-family games make delivery a queue, not a button (per CCCP `GameActivity.cpp:688-695`). M4A reserves the **delivery preview** + **landing zone risk** + **craft state** surfaces:

| Surface | M4A status | M7+ fill |
|---|---|---|
...
### Extraction zone + objective marker arrows (M1.5 + M7+ forward-compat)

Per CCCP `GameActivity.h:204-241` (objective arrows + landing-zone arrows + banners):

- **Objective arrow** points to the active objective from edge of screen (M1.5 baseline; M4A surface)
- **Extraction arrow** points to extraction zone when `extraction_offered=true`
- **Brain threat arrow** (M11+): points to the brain when brain takes damage
- **Last-known position trail** (M5.5+ memory): faint trail dots showing last-known enemy position when out of LOS
...
- "WEAPON LOCKED — wrong faction" warning if item belongs to enemy team (M11+ team forward-compat)
...
Cortex-Command-family games have a **base core identity** (DR-029): the command core can stay rooted in base (defensive bunker) OR uproot into an avatar body (mobile risk). M4A reserves the **Base Power Panel** slot:
...
These panels are NOT rendered at M4A; M4A just reserves the HUD slot + event categories + observe.* contracts.

### Mech bay / repair bay / salvage panel forward-compat (DR-016 + DR-018)

Cortex-family games have **persistent chassis identity**: armor scars, repair logs, pilot history, salvage workflow. M4A reserves these forward-compat panels:

| Panel | M4A status | M5+ fill |
|---|---|---|
...
| **Veteran panel** (M11+) | Reserved | Named actors + trait icons + reputation |
| **Commander dossier** | Reserved | Enemy commanders + rival AI persistence |

### Squad strip with role icons (M5+ forward-compat)
...
The user demand: simulate organs/circuits that take damage from heavy hits or explosions. M4A reserves the surface; M5+ fills.
...
- `auto-on-damage`: appears when any internal.organ_damaged or internal.circuit_damaged fires (M2.5+ producer)
...
The user demand: repetitive small arms fire causes concussion + reduced movement + vision. M4A reserves the surface; M5.8 origin model produces.
...
The user demand: oil/coolant/fuel/electrolyte drain visible. M4A reserves the surface; M5+ producer fills.
...
- Ground splatter: cosmetic stain at leak point (becomes hazardous tile in M5.7+)
- Ignition: chassis fire animation overlay (subject to reduce_motion + reduce_flash)

### Sound clip variants HUD coupling

Per damage model, every armor hit + internal hit + fluid event has a sound clip. M4A reserves the audio queue surface:

- `audio.event_requested { kind: <material_state>, surface_kind, damage_kind, position }` fires per hit
- M5.x+ cf-audio consumes; M2.5/M4A just emit the request events
...
### Death recap layered damage breakdown (M3B integration)

The M3B death recap modal (covered in M3B section) renders the full cause chain. M4A's HUD layer triggers the modal on `mission.mission_resolved.result=lost`:

- Modal opens AUTO on death event
- "Show me why" CTA expands the recap into the full chain (per M3B templates)
- The recap respects all M4A accessibility settings (text_scale, contrast, captions)
- M4A HUD does NOT render raw JSON; M3B templates render plain language

### Loadout / buy panel forward-compat (L3 surface for M8+)

Reserved L3 panel placeholder (M8 / loadout-workbench-slice-a fills it):
...
- **Package compatibility badge** (M8 cf-tools-editor forward-compat)

M4A reserves the panel slot + event categories (`equipment.loadout_*`); M8 fills the actual UI.

### Tactical HUD state machine

The HUD has explicit modes that mirror `Actor::Status` + tool state + UI overlay state. M4A enforces the state machine; downstream consumers (M5/M5.7/M7) extend it without renaming states:
...
stateDiagram-v2
  ...
  Idle --> Boarding: chassis attach init (M5+)
  Boarding --> Vehicle: chassis attached
  Vehicle --> Cockpit: camera anchor switch (M5+)
  Cockpit --> Vehicle: third-person view
  Vehicle --> Disembarking: exit chassis init (M5+)
  Disembarking --> Idle: exit complete
  Vehicle --> Ejecting: chassis stage=eject (M5+)
  Ejecting --> Idle: pilot ejected
  Ejecting --> DYING: bailed too late
  Vehicle --> Salvaging: mission_resolved + chassis_remaining (M5+)
  Salvaging --> Idle: salvage_finished
  Idle --> Switching: act.input.switch_actor (M11+)
  ...
  UNSTABLE --> DYING: HP <= 0
  Aiming --> DYING: HP <= 0
  DYING --> DEAD: 1000ms dwell OR no head (M5+)
  DEAD --> [*]
...
| `Vehicle (Light)` | All + pilot silhouette 60% + chassis silhouette + chassis modules + battery gauge | (none) | M5+; per-zone armor banners |
| `Vehicle (Medium)` | All + pilot 40% + chassis silhouette + 5 modules + fuel+heat+battery gauges | (none) | M5+; default chassis HUD |
| `Vehicle (Heavy)` | All + pilot 25% + chassis silhouette + 7 modules + fuel+heat+coolant+reactor gauges | jet info hidden | M5+; ground-only |
| `Cockpit` (first-person) | All + cockpit badge + sharper FOV tint + chassis-driven stance | minimap zoomed | M5+; camera anchor mode |
| `Ejecting` | EJECT_NOW banner pinned + dwell-bar | most other zones dim | M5+; chassis stage=eject |
| `Boarding` (chassis attach in-progress) | Chassis silhouette fade-in + caption | input rejected | M5+; 1500ms transition |
| `Disembarking` (chassis exit in-progress) | Chassis silhouette fade-out + caption | input rejected | M5+; 1500ms transition |
| `Driving` | Vehicle layout + driving controls hint | jet zone hidden | M5+ (tank/quadruped) |
| `Salvaging` | Salvage panel overlay + per-item tag-to-keep | mission ticker dim | M5+; post-mission cleanup |
| `Switching` (actor switch in-progress; M11+) | Brief 200ms ease + caption "Switched to <name>" | input rejected briefly | M11+ |
...
### Material overlay (M2 forward-compat — 5-mode integration)
...
| **pathability** | Per-team path costs / blocked nodes | Plan route around obstacles | M6+ |
| **mobility** | anchorable (green) / nohook (red) / passable (clear) | Pick safe anchor points | OpenLieroX nohook |
| **hazard** | Damage tiles (red), gas (yellow), electric (cyan), fire (orange) | Avoid danger | M5.7+ |
| **build_repair** | Where repair_fill can be placed | Build cover / patch breach | M5+ |
...
1. **Validity color** (per DR-012 ACC-A-03 + text label):
   ...
   - RED + "REFUSED: <reason>" caption: invalid target (e.g. metal_nohook for digger; out of range; friendly fire blocked at M11+)
...
Friendly-fire reticle color (M11+ teams): red tint when aim arc covers a friendly actor (per OpenSoldat `:2283`). Placeholder at M4A (team=0 only).
...
|---|---|---|
| **Incoming projectile arrow** | `combat.projectile_spawned` with trajectory toward player (M5.5+ collision tracking) | Screen-edge arrow + distance/ETA caption |
| **Incoming explosive** | grenade / rocket tracked toward player vicinity (M5.5+) | Screen-edge arrow + "INCOMING EXPLOSIVE" banner + sound + haptic |
| **Dropship vector** | dropship trajectory toward player vicinity (M7+) | Screen-edge arrow + "DROPSHIP INCOMING" banner |
| **"You were heard" badge** | `equipment.alarm_registered` with player as source + enemy hearing in range (per HUD-03 acceptance + M1.5 ReactiveGuard hearing) | Brief badge near reticle + caption + sound |
...
| **Hazard contact warning** | walking toward hazard tile (M2 contact predicted) | Tile pulse + caption "HAZARD ON PATH" |
| **Friendly-fire danger** | weapon arc covers friendly (M11+) | Reticle red tint + banner |
...
| **Reactor in danger** | M2.5 reactor pressure_state advances | Pulsing reactor HP bar + REACTOR_* banner |
...
- Caption text uses plain-language templates from M3B's renderer + `docs/plan/spec/death-recap-ux-contract.md`
...
DR-009 (command UX) is OPEN; M4A ships the **overlay surface** so future M4B + M7+ can layer command UI without renaming controls. M4A's slowdown is also the **accessibility comfort path** (`game_speed_assist` per ACC-A; per Game Accessibility Guidelines).
...
DR-012 closes at M4A with these settings flags. M4A's `Settings` struct is the **authoritative source** consumed by cf-ui rendering, cf-app input bridge, cf-control observe surface, and run-bundle evidence. Future T-ACC-PLUS (BP9..BP12) extends without renaming any field.
...
- Settings reachable from: first-run wizard (M4A debug menu), pause overlay, settings tab (M8 shell UI ladder), cfctl, hub (M8+)
...
|---|---|---|
| `cf-ui` | MODIFY | **HUD root** with 6-zone layout (status / ammo / objective+timer / event-ticker / silhouette+module-strip / banner-stack). **Bevy UI scale** (`UiScale` resource driven by `apply_ui_scale_from_settings`). **Palette helpers**: `palette_text` / `palette_strip_bg` / `palette_banner_bg` (swap to white-on-black when `Settings.high_contrast=true`). **Status banner stack** with 3 severity tiers + ASCII icon glyphs `[!!]/[!]/[*]`. **Caption strip** (upper-right; `Display::Flex` when queue non-empty + `Settings.captions=true`). **Tool-validity reticle** widget. **Material overlay legend** (M2 forward-compat — 5-mode legend). **Body silhouette** widget (per-zone HP tinting via `BodySilhouette` projection). **Module strip** (up to 5 modules with ammo + power-state + bound_zone fields — M5 forward-compat). **Reactive data binding** via Bevy Changed<T> queries (re-render only on observation diff). **Focus ring** + `HUD_FOCUSABLE_NODES` canonical 12-id list (also lives in `cf_control::engine::HUD_FOCUSABLE_NODES`). |
| `cf-control` | MODIFY | `act.input.focus { direction: "next"\|"prev"\|"up"\|"down"\|"left"\|"right"\|"clear" }`. `act.input.mouse_click { x, y }` + `act.input.mouse_move { x, y }` + `act.input.key_press { action }` (BP3 self-play floor). `act.settings.set` with key-remap validator: rejects `key_binding_unknown_action` / `key_binding_unknown_key` / `ui_scale_out_of_range` / `hold_threshold_ms_out_of_range` with structured reason; clamps `ui_scale` at `0.5..4.0`. `observe.accessibility` exposes all 9 ACC-A flags + `key_bindings` + `focused_node` + `caption_queue`. `observe.actor.silhouette` (per-zone HP% as BodySilhouetteView). `observe.actor.module_strip` (per-module state with ammo/power/bound_zone). `cfctl ui assert <node_id> <predicate>` (NEW: M4A UI testing harness). |
| `cf-control::engine` | MODIFY | `HUD_FOCUSABLE_NODES` const — canonical 12-element list of focusable HUD node ids. Shared by cf-ui (rendering), cf-app (keyboard handler), cf-e2e (--verify-focus), cf-control live_ws_acceptance test, cf-tools-editor (M8 future remap UI). |
| `cf-actor` | MODIFY | `BodySilhouette { head, torso, arm_left, arm_right, leg_left, leg_right }` projection — placeholder=true at M4A (scalar HP scaled into 6 zones); placeholder=false at M5 when chassis attaches. `Stance::{Idle, Walking, Running, Crouching, Airborne, KnockedDown, Downed, Dying, Dead, Ejecting, Climbing, Jetting}`. `Stance::from_chassis` derivation. |
...
- `game/crates/cf-ui/src/material_legend.rs` (MODIFY from M2: 5-mode legend integration)
- `game/crates/cf-ui/src/focus_ring.rs` (NEW: visible focus ring + 12-node iterator)
- `game/crates/cf-ui/src/event_ticker.rs` (NEW: last-event ticker with plain-language templates from M3B renderer)
...
- `game/scripts/cfctl/m4a_micro_breach_readability.cfctl.json` (EXISTS — readability on M1.5 scenario)
...
Scenario: High Contrast Light variant
  ...
  Then HUD text uses pure black on solid white backgrounds (item #803 — only High Contrast Dark exists at current M4A landing)
...
Scenario: Silhouette placeholder for chassis-less actors
  ...
  And M4A renders the same widget with placeholder tint
...
### Status banners (DR-003 + M5 forward-compat)
...
Scenario: Status banner on chassis stage change (M5 forward-compat)
...
Scenario: Status banner on M2.5 reactor pressure
  Given M2.5 reactor with pressure_state=Critical
...
Scenario: Remap action groups (item #808)
  ...
  (Currently only Gameplay group is remappable at M4A; UI/Replay/Workbench groups ladder to M8+)
```
...
Scenario: Mouse click + move acts on HUD elements
  ...
  (Items #358-359 — M4A self-play sweep rule from BP3 requires these)
...
Scenario: reduce_motion suppresses animation
  ...
  And M4B's comic-noir transitions are suppressed
...
### Tool-validity reticle + material overlay (M2 forward-compat)
...
Scenario: Material overlay legend integrated
  Given the player toggles material overlay (M2 surface; 5 modes)
...
Scenario: L0 cues survive panel-closed state
  ...
  When an incoming explosive enters detection range (M5.5+ tracking)
...
Scenario: L0 cues remain visible even when L3 panel is open
  Given the player has opened the Buy/Loadout panel (L3 forward-compat at M8+)
...
Scenario: L4 surfaces require explicit player action
  ...
  And the "Show me why" CTA hands off to M3B replay viewer
...
Scenario: Idle → UNSTABLE → KnockedDown → UNSTABLE → Idle
  Given the player takes a heavy impulse (M5.5+ collision)
  When stability < 0.3
...
Scenario: Idle → Vehicle (M5+ forward-compat)
  Given the player attaches a chassis (M5+)
  ...
  Then HUD mode = Idle
  (M4A reserves the surface; M5 fills in)

Scenario: Idle → INACTIVE (tutorial-paused, M7+ forward-compat)
...
Scenario: Incoming projectile arrow (M5.5+ forward-compat)
  Given an enemy fires a projectile aimed at the player (M5.5 collision tracks trajectory)
...
Scenario: Spotted screen-edge arrow
  ...
  And dismisses 5s after LOS is lost (last-known-position trail; M5.5+ memory)
...
Scenario: Replay records slowdown segments
  ...
  And M3B replay viewer replays the segments at the original speed (deterministic playback)
```
...
### Per-limb armor strip + internal silhouette + concussion + fluid (M2.5 deep damage)
...
Scenario: Per-limb armor strip shows zone-by-zone armor
  ...
  And the pip color signature matches M2.5: Pristine (green) / Scratched (yellow) / Cracked (orange) / Critical (red+halo) / Destroyed (gone) / ChunkedOff (animated fall)
...
Scenario: Per-zone armor on chassis-bearing actor (M5 forward-compat)
  Given M5 chassis attached with 15-zone armor
  When the HUD renders the per-limb armor strip
...
Scenario: Death recap modal includes layered damage breakdown (M3B integration)
  Given a player died from layered armor pierce + organ destruction
  When mission.mission_resolved fires
  Then death recap modal auto-opens with cause chain rendered via M3B templates
  And the recap shows: "AP round → armor pierced → organ destroyed → death"
  And "Show me why" CTA expands to full cause chain
  And the recap respects all M4A settings (text_scale, contrast, captions, reduce_motion)
  And no raw JSON is shown (M3B plain-language templates always)
...
### Chassis HUD layering (M5+ forward-compat surface)

```gherkin
Scenario: Pilot-inside-chassis dual silhouette
  Given the player is the pilot inside a Medium chassis (M5+)
...
### Brain + actor switching surface (M11+ forward-compat)
...
Scenario: act.input.switch_actor cycles controlled actor (M11+)
...
Scenario: Squad strip body-switch warning (M5+)
...
### Base power + brain core panel surface (M7+ forward-compat — DR-029)
...
Scenario: Reserve power activation warning
  ...
  And M4A reserves the surface; M7+ fills the actual module-state visualization
...
Scenario: No order without feedback (M5+ command forward-compat)
  Given the player issues an order (placeholder at M4A; M5+ wires up)
...
Scenario: No material overlay permanent clutter
  ...
  And auto-show-on-demand (M2+) surfaces only when tool refusal happens
```

### Run bundle evidence + DR-003/DR-012 closure

```gherkin
Scenario: M4A run bundle proves all 14 ACC-A tests (extended)
  ...
  Then the bundle contains:
    ...
    - expected_tests = ["M4A-D01", "M4A-D02", "M4A-D03", "M4A-D04", "ACC-A-01..14"]
...
Scenario: DR-003 + DR-012 close at M4A
  Given M4A done-criteria all pass
  Then docs/plan/decisions/dr-003-body-damage-readability.md status updates to CLOSED-DIRECTION-WITH-EVIDENCE (closed_at, closed_by_milestone: M4A, closed_evidence: [...])
...
- **Comic-noir mission card styling** — M4B / BP7 (M4A ships text-only banners + plain captions; visual styling is M4B)
- **DR-019 visual direction closure** — M4B / BP7 (the comic-noir aesthetic lands at M4B)
- **DR-009 command UX** (slowdown overlay + tactical map) — M4B / M7+ (M4A has no command surface; player controls 1 actor)
- **True SDF / MSDF text rendering** — BP6+ engineering (M4A uses Bevy 0.18.1 ab_glyph TTF; works at all scales)
- **Animated UI panel transitions** (slide + skew + ease per DR-046) — M4B / juice polish pass (M4A ships static panels; item #352)
- **Per-HUD-element juice rules per DR-046 + DR-055** — M4B / juice polish (M4A ships the events + the surface; M4B layers visual punch; item #350)
- **Tier-A 11-language localization** — T-ACC-PLUS / BP9..BP12 (M4A ships English-only keyed strings; localization extension lands later without renaming the M4A surface; items #351 + #1638)
- **Settings menu full tree** (Graphics / Audio / Controls / Accessibility / Gameplay / Language / Online tabs per DR-046) — M8 / shell UI work (M4A ships ONLY the M4A acceptance settings via cfctl; item #353)
...
- **Workbench accessibility density tests (ACC-A-08)** — M8 / `cf-tools-editor` (M4A floor is workspace-wide; M8 inherits)
- **Loadout UI accessibility (ACC-A-08)** — M8 / loadout-workbench-slice-a
- **Tutorial implementation** (12-15 min "First Contract" + 8 modular labs) — M5+ / M7 (per DR-023 + DR-046)
- **Hub / server-browser / lobby / cosmetic locker / codex / photo-mode / mod manager** — M8+ / shell UI (per DR-046; M4A ships only the HUD surface)
- **Mission briefing comic panels with AI-generated art** — M8+ / DR-044 audiovisual production pipeline
- **Adaptive hints engine** (reads EnvironmentSignal + AI bot scoring) — M7+ (per DR-023)
- **Difficulty presets (Standard / Easy / Hard / Custom)** — M5+ / DR-050 onboarding (M1.5 ships 3 ai_difficulty presets; full mission-level presets are M7+)
- **Squad list HUD on hover** — M5+ (no squad system at BP3; item #360)
- **Mouse-only HUD interactions** — explicitly forbidden at M4A (DR-012 ACC-A-04: "no mouse-only traps")
...
- **M1.5 micro breach (closed)**: provides MissionState + objective banner content + timer + enemy_hp + last_event_ticker source events.
- **M2 chunked terrain (must be done)**: material_legend integrates with M2's 5-mode overlay.
- **M2.5 micro reactor defense (must be done OR concurrent)**: reactor_pressure_state drives REACTOR_PRESSURE HUD line; reactor_destroyed triggers critical banner.
- **M3A event recorder (must be done)**: M4A's `ux.*` + `accessibility.*` event families use the locked v0.1 envelope; parent_event_id chain wiring.
- **M3B replay viewer (must be done)**: M3B's plain-language templates feed the EVENT-TICKER renderer; "show me why" CTA hands off to M3B.
- **M5 chassis (concurrent OR pre-closed)**: BodySilhouette.placeholder=false when chassis attaches; module strip reads from chassis.modules; status banners (ARMOR CRACKED / JET FAILED / EJECT NOW) fire on chassis.stage_changed.
...
When M4A done-criteria all pass:

1. Update `docs/plan/decisions/dr-003-body-damage-readability.md` status → CLOSED-DIRECTION-WITH-EVIDENCE + closed_at + closed_by_milestone: M4A + closed_evidence: [BodySilhouette projection + per-zone HP rendering + status pill + banners].
...
3. Reference bundle: `prototype_runs/native/m4a_<UTC>_<hash>/`.
4. Self-play sweep includes 4 M4A rows: `m4a_acc_a_floor`, `m4a_micro_breach_readability`, `m4a_focus_traversal`, `m4a_hold_remap_settings`. All PASS.
...
- **Line 98**: HUD data structure covers health, ammo, vest, jet, grenades, bullets, weapon, fire, team, ping, status. Our M4A HUD adds: stance, stability, sharp_aim, body_silhouette, module_strip, objective, timer, reactor — Cortex-like depth on Soldat's HUD discipline.
...
- CCCP's HUD is largely hand-coded in C++/Lua with no structured observation API; AI bots cannot "see" the same HUD the player sees. M4A's `observe.actor.silhouette` + `observe.actor.module_strip` + `observe.accessibility.focused_node` fixes this: AI agents read the EXACT same projection the renderer renders.
- CCCP has NO accessibility surface (no ui_scale, no high_contrast, no captions). M4A is the first Cortex-family game with WCAG-aligned ACC-A floor.

### Decision-record alignment

- **DR-003 (Body Damage Readability / OPEN → CLOSED at M4A)**: Recommendation E (hybrid default + advanced opt-in). M4A ships the silhouette + module strip + status pill + banner stack — the full DR-003 surface. M5 chassis fills in real per-zone HP without changing the projection contract.
- **DR-012 (Accessibility Floor / OPEN → CLOSED at M4A)**: Recommendation B. 10 ACC-A tests + 9 settings flags shipped. T-ACC-PLUS (BP9..BP12) extends with cognitive + motor + hearing + reading + sensory presets without renaming the M4A surface.
- **DR-019 (Visual Direction / closed-direction)**: M4A is the readability foundation; comic-noir styling layered at M4B.
...
- **DR-023 (Tutorial / closed)**: M4A's "show me why" CTA wraps M3B cause-chain output. Hold-to-confirm protects new players from destructive actions.
- **DR-046 (Player-Facing Surfaces / closed)**: M4A is the HUD piece. Title/main/pause/settings menu trees + lobby + mod manager + photo mode all ladder up at M8.
- **DR-051 (Accessibility-Plus / closed)**: T-ACC-PLUS extends M4A with cognitive + motor + hearing presets at BP9..BP12; surface doesn't rename.
- **DR-055 (Game Feel / closed)**: M4A ships the camera-shake + flash-suppression respect for accessibility (`reduce_motion`, `reduce_shake`, `reduce_flash`). Juice rules per surface land at M4B.
...
- Animation system for UI panel slide + skew (item #352 — DR-046; defer to M4B)
- Settings menu full tree (item #353 — defer to M8 shell UI)
...
- **Reduce motion / shake / flash forgotten in downstream juice**: M2.5 + M5 + M5.5 juice paths MUST respect the flags. Owner: each milestone that introduces motion / shake / flash.
- **ANSI color codes in viewer output (cf-tools-replay-viewer)**: M3B rule; M4A's event-ticker that consumes M3B's renderer inherits the rule.
- **English string baked in instead of keyed lookup**: localization breaks. Use the keyed-strings table from `docs/plan/spec/m4a-localization-keys.md`. M4A ships English-only values; T-ACC-PLUS adds translations.
...
- **Modal pause as the only thinking room**: anti-pattern. Slowdown75 / Slowdown25 / FullPause are all reachable; DR-009 forward-compat surface lives in M4A.
...
| Comparable | Lesson distilled | M4A integration |
|---|---|---|
| **OpenSoldat** `InterfaceGraphics.pas:98` | HUD as structured data (health/vest/ammo/jet/fire/team/ping/status) | M4A's `Settings` + `observe.accessibility` round-trip uses this pattern; HUD state is testable via cfctl |
...
| **Cortex Command** `:204-241` | Objective arrows / landing-zone arrows / banners carry must-see info | M4A's screen-edge arrows + banner stack inherit this pattern |
| **Cortex Command** `Activity.cpp:676-715` | Squad leadership reassignment surprises | Squad strip (M5+) must explain leader/follower intent + warn on body-switch |
...
| **Barotrauma** (afflictions HUD) | Multi-affliction layered HUD (bleeding, wet, electrified, etc.) | M4A reserves the affliction slot under silhouette; M5.7+ fills |
| **Stationeers** (atmosphere overlay) | Atmospheric dashboard + per-tile gas readouts | M4A's material overlay 5-mode integration follows this density |
| **Powder Toy** (material lab) | Material info panel with hardness + state | M4A's material legend per mode follows this pattern |
| **Noita** (HUD) | Wand slot + gold + HP + max_HP + perk icons | Module strip up to 5 slots inherits "slot strip with state per slot" pattern |
| **OpenSoldat launcher** `Lobby/Page.tsx` | Dense rows with double-click join + compatibility check | Forward-compat for M8 hub server browser |
...
### Design rules (M4A enforces these workspace-wide)
...
4. **Command before consequence.** Orders, purchases, package publishes, joins all show predicted result + blockers BEFORE commit (forward-compat for M5+ command UX).
5. **Dense is fine when organized.** Tables with filters + badges + drawers are acceptable for server, replay, diagnostic, package, loadout surfaces (M8+).
...
8. **No order without feedback line.** Every command surface (M5+) shows visible intent + ETA.
...
11. **Replay/death recap is one-button away.** M3B recap reachable via "Show me why" CTA on death; auto-pops on mission_resolved=lost.
...
24. **Base power has its own panel.** Don't fold base state into actor HUD; base power is L2 surface (M7+).

### Forward-compat hooks (M5+ inherits these without renaming)

**Chassis / mech scale (M5+):**
- `BodySilhouette.placeholder` flips false at M5; HUD widget renders without re-implementation
- Pilot-inside-chassis dual silhouette: M4A reserves layering (pilot at 25/40/60% based on weight class); M5 fills
- Chassis silhouette projection (`observe.chassis.silhouette`) accepts humanoid/quadruped/tank shape variants
- Chassis fuel + power + heat gauges reserve slots in AMMO+ITEM zone; M5+ fills
- Module strip's 5-7 slots accept chassis modules + power_state + ammo + bound_zone + heat fields
- Affliction icons under silhouette accept M5.7+ overlays (burning, wet, electrified, poisoned, hypoxia)
- Vehicle mode forks by weight class (Light/Medium/Heavy/Drone)
- Cockpit camera anchor + first-person mech view reserved (M5+)
...
**Threat awareness + craft (M5.5 + M7+):**
- Threat-awareness cues accept M5.5+ projectile-trajectory tracking + M7+ dropship vectors + M11+ team-color (friendly fire)
...
- Last-known-position trail dots reserved (M5.5+ memory)

**Brain + base + actor-switching (M11+ + DR-029):**
- Brain-threat banner (BRAIN_AT_RISK) reserved
- `act.input.switch_actor` reserved for M11+ multi-actor control
- Switching mode (200ms ease + handoff order summary) reserved
- Squad strip with 4 hotkey slots + leader/follower badge reserved (M5+)
- Base power panel (BASE_CORE_DAMAGED, RESERVE_POWER_ACTIVE, AVATAR_EXPOSED, uproot/reroot actions) reserved (M7+)
- `act.base.uproot` / `act.base.reroot` reserved
- Mech bay / repair bay / salvage panel L3 slots reserved (M5+)
- Pilot dossier + veteran panel + commander dossier reserved (M11+)

**HUD state + mode (M4B+M5+M7+):**
...
- `INACTIVE` mode accepts M7+ tutorial-paused state
- Pause/slowdown overlay accepts M4B + M7+ command UX layered on top
- Replay/death recap CTA hands off to M3B without changing CTA contract

**Captions + comms + workbench (M8 + M9.5 + T-ACC-PLUS):**
- Captions accept M9.5+ radio + voice transmission events
...
- Workbench density (`ui_density=Compact`) is reserved for M8 cf-tools-editor density rows
- Mission director banners are reserved for M7+ scenarios (extraction, objectives, timer warnings)
- Loadout / buy L3 panel reserved for M8+ shell UI ladder + DR-046 menu trees
- Command overlay zone is reserved for M5+ squad command UX (DR-009 forward-compat)

### Closure procedure (M4A → CLOSED)

When all M4A acceptance criteria pass:

1. Update `docs/plan/decisions/dr-003-body-damage-readability.md` → `CLOSED-DIRECTION-WITH-EVIDENCE` with `closed_at`, `closed_by_milestone: M4A`, `closed_evidence: [BodySilhouette projection + per-zone HP rendering + wound count + status pill + banners + dwell timer]`.
...
3. Update `docs/plan/decisions/dr-009-...-command-ux.md` → note M4A surface present, full DR-009 closure deferred to M4B+M7+.
4. Reference bundle: `prototype_runs/native/m4a_<UTC>_<hash>/`.
5. Self-play sweep includes M4A rows: `m4a_acc_a_floor`, `m4a_micro_breach_readability`, `m4a_focus_traversal`, `m4a_hold_remap_settings`, `m4a_threat_awareness`, `m4a_captions_filtering`, `m4a_pause_slowdown_modes`. All PASS.
6. Move `specs/active/M4A.md` → `specs/done/M4A.md` per AGENTS.md workflow.
