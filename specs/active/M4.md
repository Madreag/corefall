...
(M4 is infrastructure, not directly player-facing — but it underpins every visible feature: death recap, debug, AI trust, grading, future networking.)

- **Every player action emits a typed event** into a deterministic stream. The event taxonomy (**38 categories** — 27 from baseline + 9 added at M9: `hazard`, `shield`, `thermal`, `environment`, `armor`, `internal`, `concussion`, `fluid`, `origin` + 1 added at M13: `module` + 1 added at M17: `resource`) is the single source of truth for what the sim did.
...
### M9 firehose surface — what M4 MUST handle without renaming

M9 introduces a high-density firehose of damage/destructible-terrain/hazard/affliction/atmospheric/shield/thermal/environment events. M4 LOCKS the schemas for every M9 event family so M13+M14+M16+M19+M20 ladder up additively. The locked surfaces are:

| Family | Active producers at M9 | Registered producers (later milestones) |
|---|---|---|
| `terrain.*` | terrain_carved, material_state_changed (5-tier), pixel_removed, debris_spawned, cascade_triggered, terrain_dirty_region_batch | terrain_penetration_threshold (M14), terrain_repaired (M13+) |
| `body.*` / `combat.*` | actor_status_changed, weapon_fired, projectile_spawned, projectile_hit_mo (full expanded payload — ap_factor / armor_effective_hardness / armor_absorbed_dmg / passthrough_dmg / surface_kind / layer_struck / organ_damaged_id / circuit_damaged_id), projectile_hit_terrain, wound_added (scalar M9), reactor.armor_layer_hp_changed, reactor.armor_layer_cracked, reactor.armor_layer_destroyed | wound_added (per-zone M13), attachable_detached (M13), gib_created (M13), gib_cascade (M13) |
| `hazard.*` | hazard.spawned, hazard.spread, hazard.tick (cosmetic batched), hazard.dissipated, hazard.actor_contact | (M9 producer-active; M16+ ladders advanced rules) |
| `affliction.*` | affliction.applied, affliction.tick (cosmetic batched), affliction.cleared, affliction.escalated | (M9 producer-active for cfctl-injected; M16+ ladders environmental sources) |
| `atmos.*` | atmos.pressure_changed, atmos.gas_released, atmos.breach_detected, atmos.temperature_changed (placeholder values) | (M19 fills real kernel) |
| `shield.*` | (schemas only; no producer at M9) | shield.hit, shield.depleted, shield.regen_started, shield.regen_completed, shield.disrupted (M13+) |
| `thermal.*` | thermal.signature_changed | thermal.conduction_event (M16+), thermal.phase_change (M19+) |
| `environment.*` | environment.signal_delta (M20 surface; placeholder payload at M9) | (M20 fills aggregator) |
| `armor.*` (NEW @ M9) | armor.layer_hp_changed, armor.layer_critical, armor.layer_destroyed, armor.all_layers_destroyed, armor.chunked_off, armor.debris_spawned, armor.repaired | (M13+ chassis armor producer fills with per-zone × per-layer state) |
| `internal.*` (NEW @ M9) | internal.organ_damaged, internal.organ_destroyed, internal.organ_failure_cascade, internal.circuit_damaged, internal.circuit_destroyed, internal.circuit_failure_cascade (schemas locked; cfctl can inject for testing) | (M13+ chassis/origin model fills producers) |
| `concussion.*` (NEW @ M9) | concussion.dose_changed, concussion.band_changed, concussion.ko_threshold_crossed, concussion.recovered, internal_shock.dose_changed (robot equivalent), internal_shock.module_damaged | (M17 origin reaction model fills producer at scale) |
| `fluid.*` (NEW @ M9) | fluid.leak_started, fluid.leak_rate_changed, fluid.reservoir_warning, fluid.reservoir_critical, fluid.reservoir_empty, fluid.ignition, fluid.ground_splatter_spawned (cosmetic), fluid.leak_stopped, fluid.refilled | (M13+ chassis fluid producer fills with per-reservoir state) |
| `origin.*` (NEW @ M9) | origin.shot_force_feedback (g_load_delta + concussion_dose_delta + internal_shock_module_id + leak_channel + screen_kick_intensity), origin.g_load_dose_changed, origin.helmet_breach, origin.oxygen_supply_changed | (M17 origin reaction model fills full producer; M9 schemas locked) |
| `ability.*` | (registered, no producer at M9) | (M13+ chassis abilities) |
| `chassis.*` | (registered) | armor_layer_hp_changed, armor_layer_cracked, armor_layer_destroyed, module_state_changed, fuel_state, heat_state, eject_window_open, pilot_bailed_too_late, salvage_finished, etc. (M13+) |
...
- `game/crates/cf-headless/src/parse_command.rs` (MODIFY: covers every M0..M13 control method round-trip)
...
- `game/crates/cf-replay/schemas/event/snapshot_chassis.json` (NEW — M13 forward-compat)

Documentation:
- `docs/plan/spec/determinism-island-contract.md` (NEW — names deterministic vs non-deterministic subsystems)
- `docs/plan/references/prototype-run-bundle-schema.md` (EXISTS — the canonical envelope reference; M4 spec aligns to it)
  ...
  Then system.category_baseline event fires once at tick 0 with payload:
    {
      ...
      "categories": [
        ...
        { "name": "mind",          "status": "registered", "ladder_at": "M23" },
        { "name": "collision",     "status": "registered", "ladder_at": "M14" },
        { "name": "server",        "status": "registered", "ladder_at": "M36" },
        { "name": "anti_cheat",    "status": "registered", "ladder_at": "M36" },
        { "name": "mmo",           "status": "registered", "ladder_at": "M49" },
        { "name": "material",      "status": "registered", "ladder_at": "M15" },
        { "name": "reaction",      "status": "registered", "ladder_at": "M15" },
        ...
        { "name": "shield",        "status": "registered", "ladder_at": "M13+" },
        ...
        { "name": "module",        "status": "registered", "ladder_at": "M13+" },
        { "name": "resource",      "status": "registered", "ladder_at": "M17" },
        ...
        { "name": "logistics",     "status": "registered", "ladder_at": "M25" },
        ...
        { "name": "chassis",       "status": "registered", "ladder_at": "M13" },
        { "name": "actor",         "status": "active",     "first_event_type": "actor.snapshot" },
        { "name": "ability",       "status": "registered", "ladder_at": "M13+" }
      ]
    }
  And the payload exactly matches docs/plan/references/prototype-run-bundle-schema.md § Event Category Baseline
  (Categories `hazard`, `shield`, `thermal`, `environment`, `armor`, `internal`, `concussion`, `fluid`, `origin` are NEW at M9; `atmospherics` and `affliction` upgrade from `registered` to `active` because M9 fires placeholder events. Total: 36 categories = 27 baseline + 9 M9 deep-damage additions.)

Scenario: Locked event-type taxonomy per category (M9 active surfaces)
  Given the system.category_baseline event fires
  And the per-category event-type schema list is loaded
  Then the `terrain.*` category accepts these event types (M9 locked):
  ...
  And the `combat.*` category accepts (M9 locked):
  ...
  And the `hazard.*` category accepts (M9 locked):
    spawned, spread, tick (cosmetic), dissipated, actor_contact
  And the `affliction.*` category accepts (M9 locked):
    applied, tick (cosmetic), cleared, escalated
  And the `atmos.*` (atmospherics) category accepts (M9 locked placeholder, M19 fills):
    pressure_changed, gas_released, breach_detected, temperature_changed, phase_change (M19), combustion_ignition (M19), pipe_flow (M19+)
  And the `shield.*` category accepts (M9 locked, M13+ fills):
    hit, depleted, regen_started, regen_completed, disrupted
  And the `thermal.*` category accepts (M9 locked):
    signature_changed, conduction_event (M16+), phase_change (M19+)
  And the `environment.*` category accepts (M9 locked):
    signal_delta (M20 fills payload), hazard_band_changed (M16+)
  And the `ai.*` category accepts (M9 locked):
  ...
  And the `armor.*` category accepts (M9 locked):
  ...
  And the `module.*` category accepts (M13+ chassis):
  ...
  And the `resource.*` category accepts (M17 origin reaction model — "no HP bar" survival):
  ...
  And the `fluid.*` category accepts (M9 locked):
  ...
  And the `origin.*` category accepts (M9 locked):
  ...
  And M10's plain-language renderer ships templates for every type listed
...
Scenario: Event envelope schema v0.1 is locked
  ...
  And the schema_version field is "0.1" for M4
...
Scenario: Checksum scope sim_state_v1 (M4 canonical)
  Given the default checksum scope at M4
  ...
  And the bytes hashed are (in fixed order, big-endian where applicable):
    ...
    6. terrain_integrity_grid (per dirty chunk: integrity grid f32-quantized to u8) (M9)
    7. hazard_grid (per (chunk_id, local_pos) cell: kind_u8 + intensity_q16) (M9)
    8. affliction_state (per actor: sorted Vec<(kind_u8, severity_q16, expected_clear_tick_u64)>) (M9)
    9. armor_layer_state — per actor: per-zone armor_item_id + per-layer (kind_u8, hp_i16, hardness_q16, status_u8); per-zone occupant ("equipped armor" or "none") sorted (M9 NEW)
    10. internal_organ_state — per actor (humans/androids): sorted Vec<(organ_id_u8, hp_i16, condition_u8)>; per actor (robots): sorted Vec<(circuit_id_u8, hp_i16, condition_u8)> (M9 NEW)
    11. concussion_dose_state — per actor: (concussion_dose_q16, band_u8, internal_shock_dose_q16) where applicable per origin (M9 NEW)
    12. fluid_reservoir_state — per actor: sorted Vec<(fluid_kind_u8, current_q16, leak_rate_q16, leak_active_u8)> (M9 NEW)
    13. origin_state — per actor: (origin_id_u8, g_load_dose_q16, oxygen_supply_s_q16 [if applicable]) (M9 NEW)
    14. reactor_state (hp_i16, max_hp_i16, pressure_state_u8, heat_signature_k_q16) (M9)
    15. atmospherics_state (per atm_id: moles_per_gas_q16 + temperature_k_q16 + total_pressure_q16) (M9 placeholder; M19 fills)
    16. environment_signal_state (per actor: per-slice band enums) (M20 placeholder)
    17. mission_state (current_phase, timer_remaining_ticks, objective_states[])
    18. chassis_state (None at M4; M13 fills with stance + module states + bound zones)
  And bumping the scope to "sim_state_v2" requires a migration shim
  (M3 adds terrain chunk; M4 adds actor + inventory + M9 deep damage firehose; M13 fills chassis_state)
...
Scenario: Cosmetic event types excluded from determinism scope
  ...
  And these event types are flagged cosmetic by default:
    ...
    - shield.hit ripple (M13+ visual cosmetic; the shield.hit GAMEPLAY event is non-cosmetic)
...
Scenario: 60Hz vs 120Hz determinism (per-rate)
  ...
  And the cross-rate match is NOT required at M4 (it lands as DR-052 CI gate at M36+)
...
Scenario: snapshot_actor payload contract
  ...
  Then the payload contains: actor_id, tick, pos, velocity, aim, status, stance, hp, max_hp, stability, sharp_aim_progress, selected_slot, inventory_summary, mass, body_silhouette (placeholder=true at M4; M13 fills with chassis-backed data)
...
Scenario: snapshot_hazard_grid payload contract (M9)
...
Scenario: snapshot_affliction payload contract (M9)
...
Scenario: snapshot_armor_layer payload contract (M9)
...
Scenario: snapshot_atmospherics payload contract (M9 placeholder; M19 fills)
  ...
  (At M9: empty for default scenarios; M19 fills with real atm states)

Scenario: snapshot_environment_signal payload contract (M20 placeholder)
  ...
  (M9: empty; M20 fills with real aggregator data)

Scenario: snapshot_armor payload contract (M9 deep damage)
...
Scenario: snapshot_internal payload contract (M9 deep damage)
...
Scenario: snapshot_concussion payload contract (M9 deep damage)
...
Scenario: snapshot_fluid payload contract (M9 deep damage)
...
Scenario: snapshot_origin payload contract (M9 deep damage)
...
Scenario: Stable record_id layer (no raw pointers) — M9 firehose entities
  ...
  And the registry tracks lifecycles for these entity kinds (M4-locked taxonomy):
    ...
    - hazard_cell_id (M9: hazard tile instance)
    - affliction_instance_id (M9: per (actor_id, kind, applied_at_tick) instance)
    - shield_instance_id (M13+: per actor shield)
    - armor_layer_id (M9: per (actor_id, zone, layer_kind) instance)
    - armor_item_id (M9: per (actor_id, zone) armor item slot)
    - armor_debris_record_id (M9: per chunked-off armor piece on the ground)
    - organ_id (M9: per (actor_id, organ_kind) organ instance for humans/androids)
    - circuit_id (M9: per (actor_id, circuit_kind) circuit instance for robots)
    - fluid_reservoir_id (M9: per (actor_id, fluid_kind) reservoir instance)
    - fluid_leak_id (M9: per active leak instance)
    - atm_id (M9 placeholder; M19 fills atm units)
    - environment_signal_id (M20 placeholder)
    - module_id (M13+: chassis module instance)
...
Scenario: protocol_version bump policy
  Given an additive cfctl method addition (e.g. M13 adds act.chassis.repair)
...
### Parent-event-id cause chains (M10 forward-compat)
...
Scenario: M10 viewer can walk the chain
  Given any bundle with parent_event_id chains
  When cf-tools-replay-viewer cause-chain --event-type actor_died runs (M10 integration)
  ...
  (M4 emits the chain surface; M10 consumes it)
...
Scenario: unknown_cause marker for genuinely uncaused events
  ...
  And the M10 viewer reports the chain terminated cleanly (not a missing-parent bug)
```
...
Scenario: Cross-platform CI checksum match (DR-052 forward-compat)
  ...
  (M4 produces the surface — float-determinism rules in determinism-island-contract.md; M36+ adds the CI matrix that verifies)
  (At M4, this scenario is documented but not blocking — CI infra is M36 work)
```

## Out of scope

- Replay viewer GUI / scrubbing / cause-chain walker / debrief markdown — M10 (M4 ships the surface; M10 consumes)
...
- GGPO-style rollback netcode — DR-052 / M49 PvP arena (M4 ships deterministic surface; rollback is a network adapter on top)
- Client prediction + reconciliation for online co-op — DR-052 / M41
- Lockstep input traces for LAN co-op — DR-052 / M40
- Per-platform CI checksum matrix on Linux/Windows/macOS aarch64 + x86 — DR-052 / M36+ CI infra (M4 documents the float-determinism rules + produces the surface; the CI matrix is a workflow file added at M36)
- `cfctl test sync-drift` / `cfctl test latency-injection` / `cfctl test rollback-burst` — DR-052 / M40+
- Atmospherics / material kernel / collision / mind / mmo / chassis / affliction event PRODUCERS — those land at their owning milestones (M14, M15, M19, M23, M13, M16, M49). M4 REGISTERS the categories in system.category_baseline so the schema is locked, but does not implement the producers.
- Network event replication (event broadcast across clients) — DR-005 / M36+
- `mind.*` event family (LLM agent layer) — M23
- Replay sharing UI / community browser — BP6+ post-launch
- Save-game model (cf-save mid-mission resume) — DR-029 / M13+ (separate from replay; replay is past-only, save is resume-point)
- T-CAPTURE pipeline implementation (cf-capture frame readback) — M4 ships the contract for `captures/` in run bundles; cf-capture itself lands at BP2 (already shipped before M4)
- Replay format compression (zstd/lz4) — BP6+ optimization (M4 ships plain JSONL for inspectability; compression layers on later)
- Replay schema migration tooling — BP6+ (M4 locks v0.1; migrations land when v0.2 ships)
- Tournament-grade rollback patent licensing audit — DR-052 / M49 PvP (M4 is pre-network)
...
- **M1 actor controller (closed)**: emits `input.intent_received`, `equipment.weapon_fired`, `actor.actor_status_changed`, `actor.inventory_dropped` events that M4's verifier replays.
- **M2 micro breach (closed)**: emits `mission.*` + `ai.*` events; provides the canonical multi-objective scenario for M4's snapshot cadence test.
- **M3 chunked terrain (closed)**: emits `terrain.terrain_carved` + `terrain.terrain_dirty_region_batch` + per-chunk checksum surface that M4's verifier hashes into `sim_state_v1`.
- **M9 micro reactor defense (CONCURRENT — soft fixture dep)**: M9's reactor-defense run bundle is the canonical replay-determinism fixture for the deep M4 firehose. M4 is the FIRST active milestone in BP3 and ships first; the `m3a_replay_compare.cfctl.json` M9-fixture acceptance test is added retroactively to M4's bundle when M9 closes. M4 closes WITHOUT the M9 fixture proof; the M1+M2+M3 fixtures alone are sufficient for M4 close. M9 fixture verification happens at M9 close (M9's acceptance criteria already require headless replay parity).
- **M4A asset ledger (concurrent; soft dep)**: M4A ships the `cf-asset-ledger` crate; M4's run bundles reference ledger entries for any AI-generated assets used in capture grids. Not blocking for M4 close.
...
### Event taxonomy at M4 (status snapshot)

| Status | Categories |
|---|---|
| **active** (M0..M9 produce events) | input, control, combat, body, terrain, ai, mission, system, snapshot, determinism, ux, accessibility, performance, equipment, actor |
| **registered** (later milestones produce; M4 locks the schema) | mind (M23), collision (M14), server (M36), anti_cheat (M36), mmo (M49), material (M15), reaction (M15), atmospherics (M19), affliction (M16), logistics (M25), chassis (M13), ability (M13+) |

Don't skip the registered categories from the baseline event — they're how the schema stays additive-only. Adding a new producer at M15 doesn't require a baseline-schema bump; it just flips status from registered → active.

### Checksum scope evolution

`sim_state_v1` (M0 ships, M4 formalizes):
...
| M3 | `+ chunk_grid_checksums (per dirty chunk: blake3 of material_grid)` |
| M4 | `+ inventory_state (slot id + ammo_in_mag + reloading flag) + mission_state (status + current_objective + timer)` |
| M13 | `+ chassis_state (per-zone HP, module states, pilot state, eject ticks)` — bumps scope to `sim_state_v2` with migration shim |

Each scope name is canonical; bumping requires a migration. `_v1` accepts the M0..M4 layered additions because they're additive (extending the byte stream doesn't change the algorithm, just the input).
...
| `snapshot_chassis` | Scene start + on chassis.stage_changed + on every objective transition (M13+ when chassis is attached) |

### Replay envelope contract (v0.1 LOCKED at M0; M4 enforces)

The envelope CANNOT change shape at M4. Additive extensions to PAYLOAD are fine; envelope field additions require a v0.2 bump + migration. M14+ collision / M15 material kernels will ship payload-extensions but the envelope stays v0.1.
...
- **OpenLieroX NewNet** (`comparables/openlierox-local-audit.md:NewNetEngine.cpp:47`): had save/restore/checksum/rollback intentions but RestoreState was marked outdated. **Don't ship aspirational rollback at M4** — DR-052 directs it to M49 PvP.
- **Noita** falling-sand replay: deterministic per-tick input replay (no networking; single-player). Per `comparables/noita-grade-material-simulation-research.md`. Our M4 is equivalent for solo runs.
...
- **DR-002 (Replay/Event / closed at M10)**: M4 IS the closure milestone for hybrid event-log + snapshots. Lock the envelope at v0.1. M10 builds the viewer.
- **DR-005 (Multiplayer Posture / OPEN)**: M4 produces the deterministic surface that DR-005's lockstep + prediction + reconciliation will consume at M40+. Replay envelope size feeds the bandwidth estimate.
- **DR-018 (Death Meaning / closed)**: M4 emits the cause-chain (status → wound → projectile → fire → input). M10 walks it for death recap.
- **DR-024 (Native Engine Stack / closed)**: cf-replay is a custom crate per the "custom for hot paths" rule.
- **DR-052 (Network Sync + CLI-Testable Determinism / closed)**: M4 ships the surface for `cfctl test replay-determinism` + `cfctl test replay-bit-identical` + cross-platform float rules. CI matrix lands at M36+.

### Existing W1.2 work to credit during audit

The parallel agent's W1.2 commits already landed many M4 items. Audit must mark these `PASS (already in)`:
...
- Cross-platform CI checksum matrix (item #98 — needs M36+ CI infra; M4 documents the rules)
...
- `snapshot_chassis` for M13 forward-compat (placeholder OK at M4)
...
- **schema_version bump without migration shim**: replay breaks. Lock v0.1 at M4; v0.2 requires migration tooling that's BP6+ scope.
