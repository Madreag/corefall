...
- **M1 actor controller (must be done OR closed)**: emits `input.intent_received`, `equipment.weapon_fired`, `actor.actor_status_changed`, `actor.inventory_dropped` events that M4's verifier replays.
- **M2 micro breach (must be done)**: emits `mission.*` + `ai.*` events; provides the canonical multi-objective scenario for M4's snapshot cadence test.
- **M3 chunked terrain (must be done)**: emits `terrain.terrain_carved` + `terrain.terrain_dirty_region_batch` + per-chunk checksum surface that M4's verifier hashes into `sim_state_v1`.
- **M9 micro reactor defense (must be done)**: provides the multi-tick combat run-bundle used as the canonical M4 replay-determinism fixture (`m3a_replay_compare.cfctl.json` validates the M9 win bundle replays headlessly).
