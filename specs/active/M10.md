...
### Per-bot thinking timeline panel (M7 5-layer AI stack visualization)

M10's replay viewer gains a **per-bot thinking timeline panel** that renders the M7 smart-AI state at any scrub point. Closes the DR-022 humanlike-AI 8-criteria evidence surface.

For any tick on the scrub bar + any bot selected, the panel shows:

```
┌─ JENKINS (Medic) — tick 4271 ────────────────────────────────────────┐
│ Layer 5 (Doctrine):  "defensive"     [updated 3.4s ago, cf-llm-host] │
│ Layer 4 (HTN goal):  protect_squad → triage_medic_route              │
│ Layer 3 (Behavior):  approach_ally → treat_loop (node 2/3)           │
│ Layer 2 (Utility):                                                   │
│   ★ TriageDownedAlly(Mendez)  score=0.92                            │
│       base=0.46 × priority[9]/5=1.8 × tag_bonus=1.11                │
│     SuppressFire             score=0.61                              │
│     HoldCover                score=0.55                              │
│ Layer 1 (Reactive):  no override (no projectile within 0.2s impact)  │
│                                                                       │
│ Reason label: "chosen=TriageDownedAlly(Mendez); htn=...; bt=..."     │
│                                                                       │
│ Memory snapshot at tick 4271:                                        │
│   threat_memory: 3 entries [oldest 23 ticks; newest 4 ticks]        │
│   ally_memory:   2 entries (Mendez bleeding-3, Park nominal)        │
│   recent_events: [proj_fired, ally_downed, terrain_dirty, ...]      │
│                                                                       │
│ Chatter emitted this tick: "Treating Mendez, hold this area"        │
└──────────────────────────────────────────────────────────────────────┘
```

Reads from M8A's `reason_label_recent` + utility_candidates + htn_goal_stack + behavior_tree_node fields serialized in the snapshot envelope. M10's debrief markdown (death-recap section) renders the killed actor's full thinking timeline for the last 10 ticks before death — gives the player closure on "why did my Medic die instead of triaging me?"
