# M47 — Strategy Phase + Goals (Per-Cycle Decision Layer; Opt-In)
...
**M47 is the per-cycle strategy decision + goal stake milestone** — the player-decision layer that turns M46's upkeep mechanics into a strategic choice space. After M47, each campaign turn the player picks a Stance + Production Focus + Logistics Priority (3 axes) plus 1-3 Goals with explicit stake levels (low / medium / high). Choices modify upkeep + production + combat + diplomacy outputs for the next cycle. Goals create reward/penalty stakes that drive long-term planning.

**M47 is opt-in per server.** Default `server.ron` has `grand_strategy.enable_strategy_phase=false`; PvE Survival recommends ON; PvP defaults OFF.

M47 promise: **"every campaign turn you choose what kind of faction you are this cycle — and what you'll commit to. Goals make commitment matter."**
...
| **Research** | -10% | -5% | -5% | +5% | +30% RP/cycle (per M32) |
| **Survival** | -50% | +20% | +10% (food) | -25% | Winterize / hunker down (per M43 hunger/thirst) |
...
| **Inter-planet** | +50% supply route upkeep; supply range +200% (per M44 inter-planet transport) |
...
- AI choices visible to player via M48 intelligence (codebreaking can reveal AI's stance/goals)
- AI goals can be EXPLOITED (knowing AI committed to high-stake "eliminate player faction" lets you outlast them)

### Configurable strategy phase (per M38)
...
GrandStrategyPhaseConfig (
    ...
    ai_intelligence_reveals_choices: false,         // M48 dependency
)
```
...
| `cf-replay` | MODIFY | `strategy.*` + `goal.*` event categories (NEW) added to M4 taxonomy |
...
Scenario: AI faction picks stance + goals
  ...
  And ai.strategy_chosen fires (visible to M10 viewer for grading)

Scenario: AI intelligence reveals choices (with M48 codebreaking)
  Given player has 5 RP/cycle invested in codebreaking
  When AI's strategy phase fires
  Then M48 codebreaking attempt fires
...
- M46 (upkeep economy) — must close (M47 multiplies M46 upkeep values)
- M25 (campaign turn architecture) — must close (or M26 per Tier 2)
- M32 (tech tree for Tech goals) — must close
- M38 (settings hierarchy — opt-in toggle) — must close
- M48 (intelligence — optional; goal-reveal feature) — should close concurrent

## Closure procedure

Reference bundle: `prototype_runs/native/m47_<UTC>_<hash>/`.
...
All PASS. Move `specs/active/M47.md` → `specs/done/M47.md`.
