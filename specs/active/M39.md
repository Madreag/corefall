...
- **M25 (campaign + storyteller + boss + dialog + quest schemas locked) must close** — M39 catalogs the storyteller event family schema (per `cf-storyteller::registry`), the canonical `BossDef` schema (5 boss data files reference), the dialog tree schema (per M26 NPCs), and the quest schema (per M26 quest templates)
- **M32 (crafting + recipe schema locked) must close** — M39 catalogs the recipe schema and per-tier acceptance criteria
- **M4A (asset ledger schema locked) must close** — M39 catalogs the asset-ledger entry schema
- **M25A (narrative content schema locked) must close** — M39 catalogs codex entry + NPC dossier + mission briefing + faction lore schemas
