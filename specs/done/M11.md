...
|---|---|---|
| Body silhouette + module strip + ammo + objective + timer + last-event ticker render | PASS (already in) | cf-control HUD_FOCUSABLE_NODES (12 nodes) + cf-ui shell_components shipped at b6c5979 |
| Material overlay 5-mode toggle (off / integrity / pathability / mobility / hazard / build_repair) | PASS (already in) | M8A GPU-offload shipped; M11 reads MaterialAffordance::overlay_rgba table |
| 200% text scale + reflow via Bevy UiScale | PASS (already in) | apply_ui_scale_from_settings system + ACC-A regression tests |
| High-contrast mode palette swap | PASS (already in) | palette_text / palette_strip_bg / palette_banner_bg helpers + Settings.high_contrast |
| Color-independent state labels with ASCII glyphs `[!!]/[!]/[*]` | PASS (already in) | severity-tagged banner stack |
| 12-node focus ring (keyboard + controller) | PASS (already in) | cf-app::gamepad_focus_direction (D-Pad + triggers + analog Y) |
| Hold-to-confirm with configurable threshold | PASS (already in) | Settings.hold_to_confirm + hold_threshold_ms (default 250ms) + 5 HoldTracker tests |
| 18-action keybinding remap | PASS (already in) | Settings.key_binding_unknown_action / unknown_key rejection |
| Caption surface for audio-bound events | PASS (already in) | observe.captions queue + M4A_CAPTION_BUFFER + M4A_CAPTION_EXPIRY_TICKS |
| Reduce-motion / reduce-shake / reduce-flash flags | PASS (already in) | Settings.reduced_motion / reduced_shake / reduced_flash + M12 juice respects all three |
| Squad Strip widget (smart-AI surface) | PASS (already in) | cf-ui shell_components shipped at b6c5979 |
| Triage Window widget (compound damage stack) | PASS (already in) | TTD validator + M17 canonical TTD contract |
| Chatter caption ticker | PASS (already in) | M7 chatter scaffold + voice priority ducking spec |
| 13 status banners (HP_LOW / BLEEDING_* / LIMB_LOST / etc.) | PASS (already in) | cf-control state machine + M11 banner taxonomy |
| 9 deep damage banners (INTERNAL_ORGAN_* / CONCUSSED_* / etc.) | PASS (already in) | M9 internal damage events + cf-control banner dispatcher |
| Equipment + weapon banners (WEAPON_JAMMED / EMPTY_MAG / etc.) | PASS (already in) | M1 + M13 emission + cf-control dispatcher |
| Chassis lifecycle banners (CHASSIS_DEGRADED / MODULE_FAILED_* / EJECT_NOW) | PASS (already in) | M13 chassis state machine integration |
| `ux.banner_dismissed` event emission | IMPLEMENTED | added 2026-05-15 to cf-control::engine::refresh_hud_caches (commit b8c53e1) |
| `ux.captions_shown` event emission | IMPLEMENTED | added 2026-05-15 to cf-control::engine::refresh_hud_caches end (commit b8c53e1) |
| `ux.tool_validity_changed` event emission | IMPLEMENTED | added 2026-05-15 to cf-control::engine::tick_m6_equipment dig_validity_update block (commit b8c53e1) |
| cf-control 165/165 lib tests pass | PASS | observe_frame.schema.json refreshed via dump_schemas; all tests green |
| cf-mod ledger verify --strict clean | PASS | total=6428 fresh=6428 stale=0 drifted=0 missing=0 failed=0 (visual 6066 + SFX 242 + music 120) |
| cf-mod validate content/ pass | PASS | scanned=85 pass=1 warn=84 fail=0 (VAL-FINAL-004 satisfied) |
...
### Smart AI surfaces (HUD widgets for the M7 5-layer thinking stack)

Three new always-visible widgets that surface the M7 smart-AI state to the player non-intrusively. All three follow the ACC-A floor (200% scale + high-contrast + keyboard-navigable + screen-reader-friendly + reduce-motion respect).

#### Squad Strip widget (L2; always-visible; top-right)

Compact per-bot row with HP + role + current priority focus:
...
│   ↳ BLEEDING — 22s                      │
│ Park     ██████████ 95  ENGINEER        │
│   ↳ Repair: reactor wall (12%)          │
└─────────────────────────────────────────┘
```

Per row: name + HP bar (tinted by role: red Medic / blue Engineer / yellow Rifleman / etc.) + role badge + current activity line in plain English (drawn from the bot's `reason_label_recent` simplified). Critical-status rows (BLEEDING / DOWN / KO) pulse red at 1 Hz with ⚠ glyph. Hover with reticle (or focus via Tab → arrows) → full reason_label tooltip surfaces.

#### Triage Window widget (L0; appears only when relevant; bottom-center)

The friend-feedback contract widget. Surfaces when ANY compound damage stack is active for the player OR a squadmate within Triage Window threshold:
...
│   concussion grace         8s ← │
│                                 │
│ Auto-rescue: Jenkins 6s ETA ✓   │
└─────────────────────────────────┘
```

- **Green box** = auto-rescue arrives before TTD → no player action needed
- **Red box** = TTD beats auto-rescue → player must self-rescue OR call retreat
- **No box at all** = no compound damage stack active (default state)

Disappears 2s after compound damage stack clears. Single number on top (12 s) is the compound TTD computed from M17's canonical TTD contract. Breakdown lines show contributing affliction TTDs from M16. Auto-rescue ETA is the time the player's Medic-role squadmate will reach them.

#### Chatter caption ticker (L1; appears 3s; top-center)

Bot voice / radio chatter from M7's chatter scaffold surfaces here as captions. ACC-A: every audio chatter event has a parallel caption (no audio-only information).

```
JENKINS: "Treating Mendez, hold this area"
PARK: "Repairing reactor wall (12% complete)"
```

Maximum 2 lines visible. Fades after 3s. Audio-disabled players see captions only. Caption color tied to bot role for fast identification.

#### Priority indicator on Squad Strip (compact; non-intrusive)

Each squad strip row's right side shows the bot's current top-priority task as a small icon:
...
(Real icons are M9A asset pipeline placeholders; ASCII fallbacks for ACC-A high-contrast mode.)

When player edits Priority Table (M8 Tab overlay), the icon updates on the next AI tick reflecting the new priority focus. Visual confirmation that the change took effect.
