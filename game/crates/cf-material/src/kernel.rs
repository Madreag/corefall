...
/// (cx % 2, cy % 2). Within a color, chunks are 2+ apart so their
/// NESW neighbors (where cross-chunk emissions land) belong to DIFFERENT
...
fn dispatch_reactions_4color_parallel(
    ...
    for color in 0..4u8 {
        ...
        type ChunkResult = (i32, i32, Vec<MaterialId>, Vec<ReactionTriggeredEvent>, Vec<(i64, i64, MaterialId)>);
        let results: Vec<ChunkResult> = snapshots
            .into_par_iter()
            .map(|(cx, cy, mut pixels)| {
                let mut events = Vec::new();
                let mut cross_writes: Vec<(i64, i64, MaterialId)> = Vec::new();
                let mut cross_occupied: std::collections::BTreeSet<(i64, i64)> = std::collections::BTreeSet::new();
...
fn try_fire_reaction_snap(
    ...
    for emission_mat in &rxn.emissions {
        ...
        match candidate {
            ...
            None => {
                emission_positions.push([
                    cf_material_internal::EMISSION_DROPPED_REEXPORT,
                    cf_material_internal::EMISSION_DROPPED_REEXPORT,
                ]);
            }
        }
    }
    events.push(crate::reactions::reaction_event_with_emissions(
        rxn,
        [a_pos[0] as i32, a_pos[1] as i32],
        tick,
        emission_positions,
