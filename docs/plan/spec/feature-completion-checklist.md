...
| [x] | `T-RELEASE-S03` | Versioning axis enforced: `v0.<N>.0-bp<N>` for BP1..BP11; `v1.0.0` for BP12. Pre-release flag stays ON until BP12. | [[spec/prototype-roadmap#T-RELEASE — Per-BP Cross-Platform GitHub Releases]] | `generate_release_notes.py::parse_tag` rejects tags outside `v0.<N>.0-bp<N>` or `v1.0.0` and enforces N==BP number; release workflow sets prerelease=false only for `v1.0.0`. | - | - | - | 5 | 4 | - |  |
