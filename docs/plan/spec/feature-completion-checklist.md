...
| [ ] | `GATE-TOPIC-NET-TRANSPORT` | Networking transport library | OPEN | M9 / M10 | lightyear vs renet vs quinn for `cx-net`. Worker MUST present transport options + perf evidence + adapter-trait shape to the user before committing. | - | - | - | - | - | - | - | Decision lives outside DR system until then. |
...
| [ ] | `M6.5-S01` | `cx-ai::mind::schema`: `MindObservationFrame`, `MindTask`, `AiMindProposal`, `MindValidationResult`, `MindMemoryRecord`, `MindProviderConfig`. JSON Schemas under `corefall-game/crates/cx-ai/schemas/mind/v1/`. | [[spec/prototype-roadmap#M6.5 — LLM Mind Lab]] | - | - | - | - | - | - | - |  |
