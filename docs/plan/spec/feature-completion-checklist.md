...
| Done | ID | Feature / Requirement | Source | Evidence | H-Full | H-Quality | H-Review | AI-Full | AI-Quality | AI-Review | Notes |
|---|---|---|---|---|---:|---:|---:|---:|---:|---:|---|
| [x] | `T-RELEASE-S01` | `.github/workflows/release.yml` triggered on `v*-bp*` tag push. Build matrix covers `x86_64-unknown-linux-gnu`, `x86_64-pc-windows-msvc`, `x86_64-apple-darwin`, `aarch64-apple-darwin`. | [[spec/prototype-roadmap#T-RELEASE — Per-BP Cross-Platform GitHub Releases]] | `corefall/.github/workflows/release.yml` merged in PR #7; matrix covers Linux, Windows, macOS x86_64, macOS aarch64. | - | - | - | 5 | 4 | - | Infrastructure exists; release publication rows remain pending until tags run the workflow. |
| [x] | `T-RELEASE-S02` | `game/tools/generate_release_notes.py` reads BP tag + run-bundle summary + `summary_grid.png` + merged-PR bodies; emits release notes payload (hero image + scope summary + run-bundle stats + human-playtest survey + install instructions + determinism contract + linked PRs/vault notes). | [[spec/prototype-roadmap#T-RELEASE — Per-BP Cross-Platform GitHub Releases]] | `corefall/game/tools/generate_release_notes.py` v0.3.0 now uses the Roadmap V2 BP map, BP exemplar bundle prefixes, BP-specific smoke scenarios, strict version-axis validation, `gh pr view`/git fallback PR evidence extraction, PR body excerpts, and `BP_VAULT_NOTES` linked-vault evidence. `.github/workflows/release.yml` passes `GH_TOKEN` to the generator so GitHub Actions can read merged PR bodies. Local smoke generated BP2 notes containing `M3A`, `micro_reactor_defense`, merged PR rows, and canonical vault notes. | - | - | - | 5 | 4 | 2 | First real proof remains the `v0.1.0-bp1` release workflow run. |
...
| [x] | `T-RELEASE-S04` | Per-release artifacts: cf-app + cfctl + cf-e2e binaries + content/ + scripts/cfctl/ + summary_grid.png + exemplar run bundle, packaged per platform (`tar.zst` for Linux/macOS, `zip` for Windows) + SHA256SUMS.txt. | [[spec/prototype-roadmap#T-RELEASE — Per-BP Cross-Platform GitHub Releases]] | `release.yml` stages cf-app/cfctl/cf-e2e, content, cfctl scripts, repo docs, BP exemplar bundle, optional `summary_grid.png`, platform archives, and `SHA256SUMS.txt`. | - | - | - | 5 | 4 | - | First real proof comes when `v0.1.0-bp1` tag runs. |
| [ ] | `T-RELEASE-D01` | BP1 closure: retroactive `v0.1.0-bp1` tag from main HEAD with M1.5 summary_grid as hero; all four cross-platform binaries published; release marked pre-release. | [[spec/prototype-roadmap#T-RELEASE — Per-BP Cross-Platform GitHub Releases]] | Pending; `gh release list --repo Madreag/corefall` and `git tag --list 'v*'` returned no releases/tags on 2026-05-08. Publish before closing BP2. | - | - | - | - | - | - |  |
