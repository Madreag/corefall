...
> [!warning] Hard rules
> Everything physical collides by default unless a tested `collision_filter_reason` says otherwise. Every gameplay-physical object must also have a `PhysicalProfile` or equivalent: mass, material/composition, collision class/proxy, durability, damage routes, and relevant thermal/electrical/container/AI/debug fields. Do not brute-force all-pairs. Use broadphase, collision classes, proxies, CCD tiers, and deterministic pair ordering. Projectile-projectile collision is required for physical projectile classes; cosmetic tracers can be non-physical only if the actual projectile record still carries gameplay collision.
...
> [!important] Kickoff prerequisites
> M5.5 (Full Collision Gauntlet), M5.6 (Material Kernel), M5.7 (Hazard Package), M5.8 (Origin Resource & Overclock Pass) must be complete. Read [[spec/atmospherics-and-chemistry-model]] + [[spec/gravity-and-ballistics-model]] in full BEFORE feature work. Read [[decisions/dr-037-stationeers-grade-atmospherics-direction]] and [[decisions/dr-038-universal-gravity-and-ballistics-direction]]. Read [[research-log/2026-05-06-atmospherics-and-chemistry-stationeers-research]] for the locked numerical constants. Run [[spec/prototype-roadmap#Per-Milestone Kickoff Smoke|M5.9 Kickoff Smoke]]. M5.9 closes DR-037 and is part of DR-038 closure with M5.5.

> [!warning] Hard rules
> Real PV=nRT (`R = 8314.46`); no arcade approximations. **Stationeers-grade is the minimum bar, not the ceiling.** No subsystem reads atmosphere from anywhere except `cf-atmos` (CI grep gate). No hardcoded `9.81` anywhere in production code (CI grep gate). Combustion stoichiometry is locked per DR-037 reaction table; energy yields are deterministic. Phase change is gradual (not instant); latent heat is consumed/released. Pipe networks are first-class atmospheres; pumps/valves/regulators/filtration split networks. Doors, bullet holes, blast breaches, pipe ruptures, suit punctures, and repair patches are physical apertures. Liquids have mass/density/temperature and can jet/flood. Heat transfers through materials by conductivity/insulation, fluid movement, phase change, combustion, electrical load, and bounded ambient/radiation. Suits run real breathing math (`0.0048 mol/tick · BreathingRate · BreathingEfficiency`); helmet flush works; filter mismatch produces hypoxia warning. Per-planet ambient is an infinite reservoir with locked composition. Universal gravity is one source of truth; per-cell sampling is hot-path-cache-friendly. Determinism is mandatory: same seed + same inputs = byte-identical event stream + final state. CPU hot paths must be multicore/cache-friendly; GPU acceleration is allowed only with deterministic parity or explicitly presentation-only status.

| ID | Owns | Build | Tests | Evidence | Anti-scope |
|---|---|---|---|---|---|
| M5.9-001 atmosphere unit struct | `cf-atmos`, `content/gases/` | `Atmosphere { id, kind, volume_l, moles, moles_liquid, moles_solid, temperature_k, insulation_class, pressure_differential_max_pa, thermal_mass_j_per_k, material_shell, parent, flags }` per-gas accumulators. Gas registry: 10 launch gases (O2, N2, CO2, Volatiles, Pollutant, H2, He, O3, N2O, H2O) + 6 launch liquid mixtures with locked specific heats / latent heats / autoignition temps / molar masses. Schema leaves expansion room for more elements/materials/reactions after M8.5. | Roundtrip tests; missing-field validation; modder gas loader test. | Schema audit in run bundle; `cf-mod validate content/gases/ --strict` passes. | No GPU-only atmosphere truth data; no per-pixel atmosphere update. |
...
| M5.9-017 breach apertures | `cf-atmos`, `cf-physics`, `cf-damage`, `cf-repair` | `AtmosphericAperture { id, from, to, area_m2, normal, edge_material, open_fraction, flow_limit, liquid_limit, source_event_id, damage_stage }`. Doors/hatches/vents create stateful apertures; bullet holes/shaped cuts/blast breaches/pipe ruptures/suit punctures create damage apertures; repairs shrink/seal aperture area. | ATMOS-A-16; door-open equalization; bullet-hole leak; repair-patch shrink test; blast-breach high-flow cap test. | `atmospherics.aperture_created/changed`, `atmospherics.flow`, `atmospherics.wind_force_applied`; replay parent chain traces source projectile/tool. | No visual-only holes; no instant full-room connection for tiny holes. |
| M5.9-018 liquid pressure jets + flooding | `cf-atmos`, `cf-material`, `cf-physics` | Liquids have mass, density, viscosity, temperature, contamination, and phase. Pressure/head/gravity drives liquid jets, sprays, flooding, siphons, and mixed gas/liquid expulsion through apertures. Liquid force routes through collision impulses and material reactions. | ATMOS-A-17; pressurized tank puncture; flooded-room gravity drain; acid/coolant/fuel spray fixtures. | `atmospherics.liquid_flow`, `atmospherics.liquid_jet_force_applied`, material wetting/cooling/contamination events. | No "water level only" abstraction where jets/pressure matter; no liquid that ignores mass. |
| M5.9-019 material heat transfer + thermal tools | `cf-atmos`, `cf-material`, `cf-base`, `cf-equipment`, `cf-power` | Heat transfer routes: conduction through material shells, convection/advection by moving gas/liquid, phase-change latent heat, combustion/electrical/collision heat, bounded ambient/radiation. Player tools: heaters/coolers, heat exchangers, radiators, coolant loops, insulated panels/doors, emergency vent/dump valves, power throttling. | ATMOS-A-18; ATMOS-A-19; overheated module recovery; cold-room insulation; coolant-loop power draw; damaged insulation leak. | `atmospherics.thermal_transfer`, `atmospherics.thermal_device_tick`, `atmospherics.temperature_changed`; perf report includes thermal hot-path cost. | No cosmetic-only temperature; no hidden heat sink/source without event chain. |

M5.9 E2E:

```bash
cargo run -p cf-e2e -- --scenario m5_9_atmospherics_kernel --suite ATMOS-A-01..ATMOS-A-19,GRAV-A-01..GRAV-A-10 --write-run-bundle
...
> [!important] Kickoff prerequisites
> M5.6 (material kernel), M5.9 (Atmospherics-Grade Kernel), and M7 (mission director + Breach Contract) must be complete. Read [[decisions/dr-036-systemic-material-simulation-direction]], [[decisions/dr-037-stationeers-grade-atmospherics-direction]], [[spec/atmospherics-and-chemistry-model]], and the T-MAT comparable posture in [[comparables/noita-grade-material-simulation-research]]. Run [[spec/prototype-roadmap#Per-Milestone Kickoff Smoke|M7.5 Kickoff Smoke]]. M7.5 is not done until MAT-09 + MAT-10 + ATMOS-A-16..ATMOS-A-19 pass and the mission director can author room-state objectives.

> [!warning] Hard rules
> Stationeers-grade-or-better rules; no arcade approximation. Hull/gap/aperture/pump/vent/oxygen/pressure/fire/thermal networks must be replay-deterministic and inspectable via `cfctl observe --atmospheres`. Doors, bullet holes, blast breaches, pipe ruptures, suit punctures, and repair patches create physical apertures. Liquid jets/flooding and thermal recovery routes are part of the contract. Server-authoritative atmosphere state (DR-005 / DR-034 / DR-035). Mission director authors objectives in terms of hull/room state (`pressurize hull H3`, `vent toxic gas from hull H7`, `seal aperture A12`, `cool reactor room R4`); `cf-mod` validates hull/gap/aperture topology at scenario load.

| ID | Owns | Build | Tests | Evidence | Anti-scope |
|---|---|---|---|---|---|
| MAT-09 hull/gap/aperture topology | `cf-atmos`, `cf-mission`, `content/scenarios/` | Per-scenario hull volumes + gap/aperture connections. Each hull tracks per-gas moles, temperature, pressure, water/liquid level, fire state, toxic gas mass, thermal links, and breach apertures. Gaps/apertures carry gas flow, liquid flow, heat transfer, and force. Topology validation at scenario load. | Topology roundtrip; isolated/connected hull tests; gap/aperture-state transition tests; bullet-hole aperture test. | `atmospherics.hull_state_changed`, `atmospherics.aperture_created/changed`, `atmospherics.gap_opened/closed` events. | No real Navier-Stokes; no per-pixel atmosphere truth. |
| MAT-10 atmosphere flow + breach | `cf-atmos`, `cf-mission` | Hull flooding (water flows down through gaps), oxygen depletion, PV=nRT pressure equalization, breach handling (door/hole/rupture opening to outside). Pump + vent + door + repair entities with admin actions; flow rates from aperture area/material registry. | Flooding rate test; pressure equalization test; breach decompression test; bullet-hole leak test; pump/vent/action tests. | `atmospherics.*` events visible in M7.5 run bundle. | No normalized pressure scalar; use M5.9 PV=nRT pressure state. |
| MAT-11 fire + smoke network (atmospheric) | `cf-atmos`, `cf-material` | Fire propagates room-to-room through apertures using oxygen/fuel; smoke fills connected hulls; toxic gas migrates by pressure/aperture flow. Fires emit heat that triggers M5.6 phase changes (water -> steam) and thermal transfer. | Fire spread fixture; smoke fill fixture; oxygen depletion via fire; heat-transfer fixture. | Fire propagation events; smoke event chain; `atmospherics.thermal_transfer`. | No purely cosmetic smoke; smoke must affect breathing model. |
| MAT-12 thermal management route | `cf-atmos`, `cf-base`, `cf-power`, `cf-material` | Base modules include heaters/coolers/radiators/heat exchangers/coolant loops/insulated panels/emergency vents. Overheated room/module can be recovered by at least two techniques with distinct tradeoffs. | ATMOS-A-18; ATMOS-A-19; thermal-loop fixture; damaged-insulation fixture. | `atmospherics.thermal_device_tick`, `atmospherics.temperature_changed`, `base.module_degraded/restored`. | No cosmetic temperature; no hidden heat sinks. |
| MAT-mission director hull objectives | `cf-mission`, `cf-atmos` | Mission director can author objectives like `pressurize <hull>`, `oxygenate <hull>`, `extinguish <hull>`, `vent toxic <hull>`, `flood <hull>`, `seal <aperture>`, `cool <room>`, `restore thermal loop`. Reason labels per commander decision. | Per-objective fixture in M7.5 scenario. | `mission.objective_progress` events with hull/aperture/room id. | No campaign generator. |
| MAT-atmosphere observation | `cf-control`, `cfctl`, `cf-atmos` | `cfctl observe --atmospheres --stream --hz 5 --scope <room-id\|all>` exposes per-hull/room state, apertures, pressure/liquid jets, thermal links, and per-hull inspect. | CLI snapshot tests; stream freshness tests. | Atmosphere observation samples in run notes. | No screenshot-only debugging. |
...
```bash
cargo run -p cf-e2e -- --scenario m7_5_base_atmospherics --suite MAT-09,MAT-10,ATMOS-A-16..ATMOS-A-19 --write-run-bundle
...
| T-PERF | M0 | Add cheap counters before optimizing: frame time, sim tick cost, event volume; add terrain/AI/network counters when those systems appear. Any CPU-heavy system must declare whether it is single-thread-cheap, jobified/parallelized, background-worker, or GPU-assisted; any GPU-heavy system must report render/upload counters. Parallel code must preserve deterministic ordering where replay claims determinism. | Perf rows in `summary.json` or bench report, including CPU worker count/utilization once workers exist. |
