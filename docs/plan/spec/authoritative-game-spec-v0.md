...
| CPU/GPU performance posture | **Multicore CPU + modern GPU by default.** CPU-heavy systems must be measured, chunked/jobified/backgrounded/GPU-assisted, or proven cheap; GPU hot paths handle render/upload/visual compute without bypassing replay-authoritative state. T-PERF owns counters and acceptance evidence. | [[spec/prototype-roadmap#T-PERF — Performance Targets, Multicore CPU, And GPU Budgets]]. |
...
| Primary launch modes | **Solo + LAN co-op + online co-op + community-hostable public PvP arenas + persistent MMO shards** (DR-005 closed). Solo and private LAN/co-op rooms work with no account and no live-service dependency; public shards require an account (local or pluggable provider). Server-authoritative simulation; one `cf-server` binary; community-hostable by default. | [[spec/prototype-roadmap]], [[spec/server-app-architecture]], [[spec/persistent-mmo-architecture]], [[decisions/dr-005-multiplayer-posture]], [[decisions/dr-013-backend-service-scope]], [[decisions/dr-034-dedicated-server-application]], [[decisions/dr-035-persistent-mmo-architecture]]. |
| Dedicated server app | **`cf-server` is a full-product artifact** (DR-034). Single binary, multi-mode (`coop_room`/`pvp_arena`/`lan_room`/`mmo_shard`/`lobby_directory`); same Rust workspace; same sim path as the client; Linux + Windows; reference Docker image; documented hosting guide. | [[spec/server-app-architecture]], [[decisions/dr-034-dedicated-server-application]]. |
| Persistent MMO mode | **MMO shard is a full-product target mode** (DR-035). Bounded shard-with-portal model (NOT seamless world); 50-200 concurrent target; community-hostable; persistent terrain/bases/veterans/factions/commander memory; account required for public shards, NOT for private LAN/co-op. **Not subscription-funded.** M12 proves readiness and reopens the DR if it fails. | [[spec/persistent-mmo-architecture]], [[decisions/dr-035-persistent-mmo-architecture]]. |
...
| Systemic material simulation | **Hybrid systemic materials are a core feel pillar** (DR-036): bounded active-region per-pixel CA kernel (`cf-material`) + **Stationeers-grade-or-better atmospherics/thermal simulation** (`cf-atmos`, DR-037) + data-driven reaction table + per-actor affordance/affliction layer. Curated **17-material launch set** (air, dirt/sand, rock/concrete, metal, wood/organic, water, steam/mist, smoke, fire/heat, oil/fuel, acid, toxic sludge/liquid, toxic gas, lava, blood/vomit, electricity charge, pebble/debris). Expansion materials/elements/gases/reactions are encouraged but gated behind material lab (M8.5) evidence, inspect overlays, AI affordances, replay events, accessibility captions, balance notes, and perf proof. AI material competence required (M6.6). Replay-deterministic; server-authoritative. | [[decisions/dr-036-systemic-material-simulation-direction]], [[decisions/dr-037-stationeers-grade-atmospherics-direction]], [[comparables/noita-grade-material-simulation-research]], M5.6 MAT-01..03 + MAT-06 + MAT-13, M5.7 MAT-04/05/07 + MAT-08-stub, M5.9 ATMOS-A, M6.6 AI-MAT-01..AI-MAT-08, M7.5 MAT-09/10, M8.5 MAT-11/14. |
| Pressure, thermal, and atmosphere simulation | **Stationeers-grade is the floor**: PV=nRT atmospheres, per-gas moles, temperature, phase change, combustion, pressure apertures from doors/vents/weapon holes/blast breaches/pipe ruptures/suit punctures, gas wind, liquid jets/flooding, heat transfer through materials, and player heating/cooling techniques. CPU deterministic truth is required; hot paths must be multicore/cache-friendly; GPU acceleration can be added where deterministic parity or non-authoritative presentation status is proven. | [[spec/atmospherics-and-chemistry-model]], [[decisions/dr-037-stationeers-grade-atmospherics-direction]], [[spec/prototype-roadmap#M5.9 — Atmospherics-Grade Kernel]], [[spec/prototype-roadmap#M7.5 — Base Atmospherics (Extended For Stationeers-Grade Per DR-037)]]. |
...
|---|---|---|
| Physical profile contract | Every gameplay-physical thing has mass plus material/composition properties. Units, limbs, armor, weapons, equipment, loose items, projectiles, debris, base modules, doors, turrets, shields, mech parts, containers, batteries, terrain materials, and mission-critical objects own or derive a `PhysicalProfile` with collision, durability, damage-route, thermal/electrical/pressure, AI affordance, and inspect/debug fields where relevant. | Cosmetic particles, UI-only markers, pure sensors, and non-gameplay VFX can opt out only with explicit tested reason. |
...
| What backend scope is worth building now? | **Closed** (2026-05-05): full backend service spine + dedicated server app + community hosting + optional first-party services. Local-first remains the default for solo/private LAN play; `lobby_directory`, account adapter, persistence, anti-cheat foundation, and observability ship at launch. Steam/EOS/PlayFab/Unity Multiplay are optional adapters. | BACK-SCOPE/BACK-A/SERVER-001..SERVER-016/MMO-001..MMO-012 evidence; [[decisions/dr-013-backend-service-scope]]; [[decisions/dr-034-dedicated-server-application]]; [[decisions/dr-035-persistent-mmo-architecture]]. |
