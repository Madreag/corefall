...
| The first fixture file now instantiates nine LOAD-A loadouts with actual Base/Coalition/Ronin item IDs, expected warnings, manual badges, and test coverage. | [[references/equipment-loadout-fixtures-slice-a]], `cortext_command_vault/references/equipment-loadout-fixtures.slice-a.json` | LOAD-004/005/006/007 can start from shared data instead of each prototype inventing its own item set. |
| The first manual patch layer now covers 14 high-priority records: replacement/catalog policy, internal turret/payload classification, scripted support/mobility/sensor contexts, and launcher bot-label gating. | [[references/equipment-manual-overlay-patches]], `cortext_command_vault/references/equipment-overlay-manual-patch.base-coalition-ronin.json`, `research_tools/equipment_overlay_check.py` | Prototypes should load generated seed, then manual patch, then fixture data; generated files stay reproducible. |
| A merged preview now applies the manual patch and produces fixture diagnostic reports for all nine LOAD-A fixtures. | [[references/equipment-overlay-merged-preview]], `cortext_command_vault/references/equipment-overlay-merged.preview.json`, `research_tools/equipment_overlay_merge.py` | LOAD-A consumers can use one patch-applied artifact while the seed and manual patch remain separately reviewable. |
| The first provenance workbench view turns fixture item rows into consumer-facing diagnostics: 33 LOAD-A item rows, 25 unique fixture items, 0 missing fixture items, attention-field counts, warning fix queue, source highlights, and AI/UI/modding/balancing/replay implications. | [[references/equipment-provenance-workbench-view]], `cortext_command_vault/references/equipment-provenance-workbench-view.slice-a.json`, `research_tools/equipment_provenance_workbench_view.py` | LOAD-001C gives the buy/loadout UI and package workbench an immediate provenance panel instead of asking designers to read raw JSON. |
| AI-H equipment scenario seeds now convert the nine LOAD-A fixtures into bot tests. | [[references/equipment-ai-scenarios-slice-a]], `cortext_command_vault/references/equipment-ai-scenarios.slice-a.json`, `research_tools/equipment_ai_scenarios.py` | AI work can test weapon choice, breach tools, support use, explosive refusal, target classes, pickup value, and negative loadout preflight using the same item IDs and diagnostics as the loadout UI. |
| Generated AI summary seeds now turn all 106 role-card rows into bot decision contracts. | [[references/equipment-ai-summary-seed-slice-a]], `cortext_command_vault/references/equipment-ai-summary-seed.slice-a.json`, `research_tools/equipment_ai_summary_seed.py` | AI, UI, package diagnostics, balance, replay, and backend/session code now share `claim_state`, decision inputs, blackboard keys, reason labels, event families, scenario links, and source confidence before any item is promoted to bot-safe. |
...
| Package-builder diagnostic expected output now turns fixture warnings into dev/local/published mode verdicts and bot-assignment verdicts. | [[references/equipment-package-diagnostics-slice-a]], `cortext_command_vault/references/equipment-package-diagnostics.slice-a.json`, `research_tools/equipment_package_diagnostics.py` | Workbench tests can prove warnings are visible, package-mode gates differ, and unsafe bot defaults are blocked without blocking private player experimentation. |
| Generated role cards translate the merged overlay and selected non-catalog CCCP examples into 106 system-readable item cards. | [[references/equipment-role-cards-slice-a]], `cortext_command_vault/references/equipment-role-cards.slice-a.json`, `research_tools/equipment_role_cards.py` | LOAD-009 now has a repeatable artifact for buy/loadout UI, AI harness, package-builder diagnostics, balance, replay/backend, and modding consumers. |
| Generated overlap audit finds 10 player-catalog role-signature overlap groups covering 42 catalog items; 3 are high-risk. | [[references/equipment-overlap-audit-slice-a]], `cortext_command_vault/references/equipment-overlap-audit.slice-a.json`, `research_tools/equipment_role_cards.py` | LOAD-010 now has concrete balance/UX pressure: medium assault rifles and sidearms need stronger handling, recoil, ammo, AI-policy, economy, mission-role, or cosmetic/skin decisions. |
...
| Renderer view model generates concrete LOAD-R fixture output. | [[references/equipment-role-card-renderer-view-slice-a]], `cortext_command_vault/references/equipment-role-card-renderer-view.slice-a.json`, `research_tools/equipment_role_card_renderer_view.py` | The first renderer fixture exposes 63 unique player-catalog rows, 106 workbench rows, 5 detail drawer examples, 10 overlap rows, 9 fixture summaries, and a LOAD-R coverage map. |
...
| Miner (per [[spec/mining-and-extraction-model]] / DR-041) | Sample → drill → extract → refine → smelt ore. Robot origin preferred for vacuum + heat extremes; human + drill rig for surface; mech for hard rock. | Sample scanner, drill (origin-rated), drill bits, ore cargo pack. | Refining unit, smelter unit, voice + radio comms (per [[spec/comms-voice-and-radio-model]]). | Origin fitness for environment, ore hardness vs bit hardness, drill overheat, cargo overflow, deposit exhaustion, return trip. |
...
| Mining tool (per [[spec/mining-and-extraction-model]] / DR-041) | sample scanner, drill (light/medium/heavy/vacuum-rated), drill bits (per ore hardness class), refining unit, smelter unit, ore cargo pack. | ore_hardness_curve, drill_power_w, bit_hardness_class, drill_overheat_threshold, cargo_mass_kg, vacuum_rated_flag, origin_gating, power_source. | Sample range, drill speed, bit overheat threshold, cargo capacity, vacuum compatibility, origin requirement (e.g., vacuum-only-robot). |
| Voice + radio (per [[spec/comms-voice-and-radio-model]] / DR-043) | helmet voice pickup, throat mic, bone conductor, handheld VHF, backpack VHF, HF transceiver, satellite uplink, antenna whip/dipole/yagi-directional/ground-plane, jammer, encryption module. | frequency_band, frequency_hz, encryption_key, antenna_pattern, tx_power_w, rx_sensitivity_db, occupies_slot, origin_gating, power_source. | Range band, LOS-vs-multipath, encryption status, frequency-tuning UI, antenna directional gain, origin requirement (humans equip; robots built-in; android modular). |
...
| LOAD-005 | Load `cortext_command_vault/references/equipment-loadout-fixtures.slice-a.json` into [[spec/ux-wireframes-slice-a]] buy/loadout prototype screens. |
...
| LOAD-009A | First renderer view model exists in [[references/equipment-role-card-renderer-view-slice-a]] and `cortext_command_vault/references/equipment-role-card-renderer-view.slice-a.json`; next consume it in an actual UI/workbench prototype. |
...
| LOAD-011 | Keep generated [[references/equipment-consumer-traceability-slice-a]] and `cortext_command_vault/references/equipment-consumer-traceability.slice-a.json` current as LOAD-W, AI-H-LOAD, PACK-014C, LOAD-010C, and REC-A-LOAD add real consumer evidence. |
| LOAD-W-010A | Consume [[references/equipment-trace-tab-view-slice-a]] and `cortext_command_vault/references/equipment-trace-tab-view.slice-a.json` in the workbench prototype as the source for item trace rows, fixture tabs, diagnostic trace rows, gap badges, and open targets. |
