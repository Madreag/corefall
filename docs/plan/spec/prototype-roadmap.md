...
| **BP2** | Terrain & Replay Build | M2 + M2.5 + M3A | DR-002 lean refresh (M3A locks event taxonomy + headless replay; M3B closes); DR-007 launch-material set frozen for the chunked-terrain era | `cf-e2e --script m2_dig_concrete_refuse_metal` + M2.5 micro reactor defense + replay verifier | <span class="cc-flag cc-green">CLOSED</span> (M2 + M2.5 + M3A engineering-complete; bundles `m2_*` + `m2.5_*` archived; cf-headless replay verifier proves determinism end-to-end; T-RELEASE tag pending merge) |
| **BP3** | Combat Readability Build | M3B + M4A + M5 | DR-002 closure; DR-003 closure; DR-012 ACC-A floor closure; DR-014 / DR-021 chassis grammar closure | `cf-e2e --script m5_chassis_wreck_eject_{win,loss}` + `cf-e2e --script m5_chassis_salvage_roundtrip` + `cf-e2e --script m4a_micro_breach_readability --ui-scale 2.0 --high-contrast` | <span class="cc-flag cc-green">CLOSED</span> (M3B closed via commit 50af435; M4A closed via PR #27; M5 closed on this branch — 3 chassis archetypes + body graph + layered armor + module state machine + pilot binding + eject + salvage + save/load. Self-play sweep 19/19 PASS, LLM-graded 9.23 + 9.13 PASS.) |
| **BP4** | Physics Sandbox Alpha | M5.5 + M5.5.5 + M5.6 + M5.7 + M5.8 | DR-033 closure; DR-036 (M5.6/M5.7) implementation slices; DR-007 implementation specifics partial | M5.5.5 micro sabotage + per-milestone gauntlets (COLL-001..012, MAT-01..03) | <span class="cc-flag cc-yellow">ACTIVE</span> |
