...
| M9 dedicated server app | `server.*` events captured for boot, mode selection, config load, client admit/drop, persistence cadence, health/readiness probes, drain shutdown; SERVER-001..SERVER-016 acceptance suite passes against checked run bundles. |
