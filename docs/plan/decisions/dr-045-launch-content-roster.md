...
- Submit content via Steam Workshop (per DR-059) with one-button publish.
...
| Modding workshop | Steam Workshop integration + community-hostable mod repository per DR-059. |
...
| Replayability | Per DR-062, retention requires per-mission variety. 30+ launch missions + procedural contracts + Bunker Defence + creator tools + match grammar = hundreds of unique sessions. |
