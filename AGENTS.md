...
- **T-RELEASE tag mandatory from BP1 onward**: every BP closure produces a tagged GitHub Release using the SemVer prerelease channel scheme:
  ...
  The release ships cross-platform binaries (Linux, Windows, macOS x86_64, macOS aarch64) plus the BP's exemplar run bundle + `summary_grid.png` + auto-generated release notes (run-bundle stats + AI-Agent Self-Test Report + determinism contract + SHA256SUMS). The tag is pushed AFTER the BP's PR is merged into `main`. Quality is communicated via the SemVer channel suffix in the tag name itself (`-prealpha`, `-alpha`, `-beta`, `-rc`, or no suffix for `v1.0.0` GA); the GitHub `prerelease` flag is intentionally NOT set because it would double-count the quality signal AND hide releases from the `/releases/latest` endpoint + the repo homepage Releases sidebar widget (both filter out prereleases by GitHub's definition). The `generate_release_notes.py` parser enforces the channel-boundary contract and rejects mismatched tags (e.g., `v0.4.0-prealpha` because BP4 must ship under `alpha`). The legacy `v0.<N>.0-bp<N>` form is still accepted for backward compat with already-published releases. See `docs/plan/spec/prototype-roadmap.md` §T-RELEASE for the full contract.
