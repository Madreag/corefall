...
- **HARD RULE — Comments stay SHORT.** Default to none. **One line max** for non-obvious WHY. **ZERO multi-line narratives.** **ZERO restating what the code does.** **ZERO milestone/spec/PR/audit/bugbot references in code bodies.** Doc-comments document API contracts only and stay ≤2 lines. If a comment would need more than one line, the code needs a better name or a smaller function — fix that instead. Long comments inflate LOC, pollute diffs, and rot when code changes. **Never use `///` block-comment narratives just because Rust supports them. Never use `// **AUDIT TAG**` style. Never reference the session/bugbot/PR that introduced the change.** When in doubt, delete the comment.
- **Files stay UNDER 1000 LOC (ideal) / under 2000 LOC (hard ceiling).** AI agents read 1000-LOC files cleanly, ~2000 is the upper bound, anything higher is allowed ONLY for a documented perf or safety reason in the file header. Past 800 LOC look for natural module boundaries proactively. Past 2000 LOC split unless the file-header reason justifies it.
- **All tuning constants live in `content/` JSON/RON.** Code reads them via loaders with hardcoded fallback only for boot. Add a JSON file before adding a `const X: f32 = ...` in source. **Player/admin/modder-editable settings live under `content/settings/` organized by topic** (graphics, audio, controls, gameplay, sim, network, debug); engine-internal tuning lives under `content/<crate>/` (materials, reactions, weapons, etc.). Loaders MUST `tracing::warn!` on parse failure — never silently fall back.
