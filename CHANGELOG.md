...
- **Recorder helpers** added to `cf-replay::Recorder`: `events_since(after_idx) -> Vec<Event>` and `event_log_len() -> usize` so cf-app's keyframe pump doesn't clone the full event log every frame.
