...
**Parallel-deterministic sim. Byte-identical across machines. 200 actors at 60 Hz.**

A 2D side-view physics sandbox where every gas, grain, bullet, body, world, transmission, and joint is real. Bunker Defence is the flagship mode: attackers breach, defenders hold the command core, and every pressure seal, bullet-punched aperture, liquid jet, thermal leak, radio shadow, projectile, wound, fire, severed limb, and collapsing room is part of the same replayable simulation — running on a parallel-deterministic ECS that ships byte-identical replays across Linux x86_64, Windows 11, and macOS aarch64, on workstation 9950X3D clients, Mac mini M4 Pro servers, and Linux VPS shards.
...
[![Reference Platform](https://img.shields.io/badge/Reference_platform-9950X3D%20%2B%20RTX_5090-76B900?style=flat-square&logoColor=white)](#)
...
> [!important]
> **Where we are (2026-05-15):** `M11` (Readability + ACC-A Floor) closed — 25-scenario verdict table all PASS / IMPLEMENTED + 165/165 cf-control lib tests + cf-mod ledger verify clean. M1 → M11 shipped: M1 (Actor controller) + M2 (Micro Breach) + M3 (Pixel Terrain) + M3A (Determinism) + M4 (Event Recorder) + M4A (Asset Ledger) + M5 (Deep Damage) + M6 (Actor Depth) + M7 (Smart AI) + M8 (Tactical Overlay) + M8A (Parallel Sim + Networking) + M9 (Reactor Defense) + M9A (Tier 1 SVG Pipeline) + M10 (Replay Viewer) + M11 (HUD Readability + ACC-A).
>
...
| **AI as teammate and rival — 8-criteria humanlike bar** | Bots reason, plan, panic, recover, and explain themselves through replay-visible reason labels. **6 archetypes** (Medic / Engineer / Rifleman / Sniper / Assault / Spotter). **5-layer thinking stack** (Reactive → Utility → Behavior Tree → HTN Planner → optional LLM Mind) runs every tick at 80 µs per bot on 16 cores. **Personality traits** affect AI (brave, coward, sharp_shooter, paranoid). Per-actor **BotMemory** (16 KB perception grid + threat memory + ally memory + recent events) — bots remember where they got shot at and avoid the same kill zone. Optional async LLM "mind" layer proposes doctrine without blocking the simulation. **Persistent AI commanders** remember player tactics across missions. **Owned by M2.2B + M6 + M6.5 + M6.6 + M7 + M8A + M11 + M23.** |
| **Smart commandable squad — three-layer control model** | **Layer 1 — Autonomy mode** per actor: FullAuto (default; minimal HUD chatter; bots auto-act) / Standard / Manual. **Layer 2 — Priority Table**: 22-task × 1-9 weight grid per actor (RimWorld + ONI pattern); 6 role templates pre-fill; player edits via Tab overlay; persists across missions. Feeds AI utility scorer as multiplicative weight prior. **Layer 3 — Live Orders**: Q-hold context wheel (door = breach/stack/mirror; ally = treat/cover/follow; etc.) + single-key panic (M = medic-on-me / R = repair-this / G = regroup) + MMB tag-and-mark (priority signal) + Tab tactical overlay with **Plan Composer** (up to 8-step queued plans per bot; Door Kickers + Ready or Not pattern). Bots **speak what they see** (M7 chatter scaffold; M37 production voice). Every action emits a **reason_label** ("Triaging Mendez because score=0.92, priority[Triage]=9, role=Medic, tag_bonus=1.11"); F1 "Why?" key surfaces it in-mission. M10 replay viewer renders the full thinking timeline per-tick. **Owned by M7 + M8 + M8A + M11.** |
| **Auto-triage + auto-repair — survival math players can track** | Compound damage TTD (time-to-death) published per origin × per difficulty (M17 canonical table). HUD **Triage Window widget** shows compound TTD as one integer; per-affliction breakdown on demand. **Medic auto-triages** allies entering Downed status: reach ≤ 6s + apply ≤ 8s + stabilization-window contract. **Engineer auto-repairs** chassis modules + reactor wall breaches mid-combat. Compound damage stack floor: ≥ 8s at p99 at default difficulty (the player-reaction-time guarantee). Veteran: 5s. Hardcore: real-physics TTD only. **Owned by M7 + M11 + M13 + M16 + M17.** |
...
| **Replay determinism, end-to-end** | Same seed + same inputs = byte-identical event stream. 27+ event categories with `parent_event_id` cause chains. Per-tick blake3 checksums anchor the contract. **Cross-platform** (Linux x86_64 + Windows x86_64 + macOS aarch64) + cross-rate (60Hz + 120Hz) verified by CI gate `m8a_cross_os_determinism.sh`. Death recap walks the full chain from death event back to player's last action. **Owned by M3A + M3B + M8A.** |
| **Parallel-deterministic ECS — 200-actor scenes at 60 Hz** | M1-M8 single-thread sim becomes parallel-deterministic at M8A. Bevy ECS scheduler runs actor / AI / projectile / terrain / mission systems concurrently on 16 cores via snapshot-read / compute-parallel / commit-serial (the same pattern Factorio uses). 50+ actors + 200 projectiles + 100 hazard pixels + reactor armor + per-pixel terrain integrity in one scene at p99 ≤ 16.6 ms on the reference platform — **200-actor stress test holds 60 Hz**. Determinism preserved across thread-scheduling variance: pre-rolled RNG, ordered chunk-merge, RecorderShard per worker with canonical merge. No `f64` / no `thread_rng()` / no default-hashed `HashMap` in sim crates (CI-enforced). **Owned by M8A.** |
| **GPU offload — cosmetic richness without sim cost** | Particle effects, debris rendering, damage numbers, blood splatter, shell ejection — all GPU-side via wgsl compute shaders + fragment shaders, all `cosmetic: true`, all droppable under backpressure with zero impact on sim. Material overlay (5-mode integrity / pathability / mobility / hazard / build_repair) runs per-fragment on the GPU. Chunked terrain uses one `Texture2DArray` (not one texture per chunk). Player sees 10,000 sparks during a reactor breach; sim doesn't even know. **Owned by M8A.** |
| **Server owns truth — same binary, 5 modes, multi-tier hardware** | One `cf-server` (via `cf-headless serve`) binary, 5 launch modes (`coop_room`, `pvp_arena`, `lan_room`, `mmo_shard`, `lobby_directory`). 3 anti-cheat profiles (`casual`, `competitive`, `tournament_strict`). 4 mod trust tiers (`vanilla`, `verified`, `community`, `experimental`). **4 server hardware tiers**: workstation x86_64 + RTX 5090, Apple Silicon Mac mini M4 Pro / M5+, Linux VPS (no GPU required), Apple Mini Lab cluster (3-5 nodes for persistent-MMO mesh dev). Community-hostable. No proprietary cloud lock-in. Server-authoritative-entity model + semantic-terrain-events + LAN lockstep + internet rollback. **Owned by M8A + M9 + M10 + M11 + M12.** |
| **Networking — QUIC, lockstep on LAN, rollback on internet** | `cf-net` ships at M8A on QUIC (via `quinn`) for the transport, with WebSocket fallback for future browser spectator clients. **LAN mode**: deterministic lockstep — all clients sim-step locally with identical inputs; byte-identical output. **Internet mode**: 6-frame rollback netcode — client predicts, server confirms, client rolls back + resimulates on misprediction (re-uses the same deterministic sim core, no parallel rollback codepath). Wire protocol locked at `prototype-net-frame.v0.1`, additive-only extensions. Semantic terrain events (`terrain.chunk_mutated` with `post_state_checksum`) replace CCCP-era bitmap deltas (~5x bandwidth savings). **Owned by M8A.** |
...
| **M7** | ✅ | BP2 | AI Archetypes + Mission Director + **Smart Commandable AI** | **6 archetypes** (5 + Medic) · **5-layer thinking stack** (Reactive + Utility + Behavior Tree + HTN + LLM-prior) · **per-actor Priority Table** (22 tasks × 1-9 weight; 6 role templates; 5 quick presets) · **chatter scaffold** (placeholder TTS + captions; M37 ships production voice) · reason_label on every action · auto-triage + auto-repair as first-class contracts · 20+ traits · 3 doctrines · 3-faction matrix · multi-objective DAG · 4-phase pacing |
| **M8** | ✅ | BP2 | UX + Camera + Debug + L10n + Accessibility + **Squad Control Surfaces** | **Tab tactical overlay + Priority Editor + Plan Composer (up to 8-step queues)** · **Q-hold context wheel** (door/ally/enemy/terrain/hazard/module contexts) · **single-key panic (M/R/G)** · **MMB tag-and-mark** · **"Why?" key** surfaces reason labels · 10+ HUD widgets · pie menu · 7 debug overlays · 4 color-blind modes · photo mode · replay scrubber · killcam |
| **M8A** | ✅ | BP2 | **Parallel Determinism + GPU Offload + Server Architecture** | **Massive performance + networking foundation.** RwLock<EngineMutable> → Bevy ECS scheduler · snapshot-read / compute-parallel / commit-serial · cf-net (QUIC + LAN lockstep + internet rollback + `cf-headless serve`) · GPU cosmetic offload (compute shader particles + fragment-shader material overlay + Texture2DArray terrain) · semantic terrain events (no bitmap deltas) · Powder-Toy-derived snapshot envelope · 4 server hardware tiers (9950X3D+RTX5090 / Apple Silicon M4 Pro+ / Linux VPS / Apple Mini Lab) · M1-M6 backfill gate (every shipped subsystem retrofitted to ECS with byte-identical replay) · 3 CI gates (perf-budget + cross-OS-determinism + M1-M6-backfill) |
| **M9** | ✅ | BP2 | Micro Reactor Defense Fun Slice | 60-90s defend scenario · 5-tier terrain HP · 3-layer reactor armor · trench gameplay · **runs on M8A parallel foundation (firehose stress test)** |
...
| **M24A** | 🔄 | BP6 | VFX Tier 1 | 600+ particle configs · 80+ textures (impact / spark / explosion / debris) · **Tier 1 VFX content shipped: 120 Particle entries + 64 VfxFrame animation frames (4 per effect across muzzle_flash/explosion/impact_burst/blood_splat) + 96 VfxDecal entries (bullet holes / scorch / blood pools / oil / acid / frost / lava / craters / footprints / chassis fluid leaks)** |
...
| **M25A** | 🔄 | BP7 | Narrative + Codex Production | 5k-word LLM-driven bible → 80k words · 600 codex entries · 24 named NPCs · **Tier 1 narrative bible authored: 8 factions + 12 worlds + 24 NPCs + 5 storytellers + 5 bosses + 12 missions + 10 origins + 300 codex + 75 achievements + 60 loading tips + credits template** |
...
| **M32A** | 🔄 | BP8 | Tier 2 ComfyUI Pipeline | 4500+ production assets · SDXL + Flux + AnimateDiff + ControlNet + per-faction LoRAs · **Tier 1 NPC portrait placeholders shipped: 44 entries (24 named + 5 storytellers + 5 bosses + 10 generic)** |
...
| **M33A** | 🔄 | BP8 | Tutorial Lab Production | 8 modular labs · First Contract · FRE wizard · adaptive hints · **Tier 1 voice prompts authored (70+ tutorial narration lines across 8 labs); 3 FRE wizard step mockups baked** |
...
| **M37A** | 🔄 | BP9 | Tier 2 Audio + Voice + Music | 7000+ voice clips (ElevenLabs / Bark / XTTS) · 30+ music tracks (MusicGen) · adaptive music engine · **Tier 1 procedural music bake landed: 120 WAVs (12 worlds + 8 factions + 5 storytellers + 5 bosses x 4 variants); 4 voice prompt manifests authored (chatter / NPC / storyteller-boss / mission-tutorial)** |
...
| **M48A** | 🔄 | BP12 | Tier 3 Polish | Top 50 Aseprite hand-polish · top 20 Spine rigs · FMOD / Kira mix · Steam Deck Verified · **Tier 1 cinematic placeholders shipped: 23 boss splashes (5 bosses x intro/phase variants) + 20 loading backgrounds + 19 marketing key art + 44 portraits** |
| **M48B** | 🔄 | BP12 | Steam Store + Marketing | Capsule art · 12 screenshots · 6 trailer types · press kit · tag taxonomy · **Tier 1 marketing placeholders shipped: 19 KeyArt entries (main cover + 8 faction + Steam capsule/hero/3 screenshots + press kit + Deck verified + 3 DLC mocks)** |
...
| **Replay determinism** | Same seed + same inputs = byte-identical event stream — on Linux x86_64, Windows x86_64, AND macOS aarch64 (Apple Silicon). 27+ event categories with `parent_event_id` cause chains; per-tick blake3 checksums anchor the contract. Debug with replay scrubbing. Network with confidence. Audit AI behavior with cause chains. *(Cross-OS determinism CI gate ships at M8A: `m8a_cross_os_determinism.sh` runs the same scenario on Linux + macOS + Windows and diffs final checksums. No f64 / no thread_rng / no default-hashed HashMap in sim crates — CI-enforced.)* |
| **Server truth, rich clients** | Server owns authoritative state; clients own immediate feel and GPU-rich presentation. Prediction is allowed, divergence is not. **Server-authoritative-entity model + semantic-terrain-events** (per the vault's multiplayer audit) — not bitmap deltas (CCCP anti-pattern), not pure deterministic-lockstep on internet (too fragile under floating-point drift). LAN gets cheap lockstep, internet gets prediction + rollback. Single player uses the same architecture in-process. The same `cf-headless serve` binary runs on workstation x86_64 + NVIDIA, Apple Silicon M4 Pro / M5+, or headless Linux VPS — all pass the same cross-OS determinism gate. |
...
| **No-compromise performance defaults** | Performance-sensitive values are config-driven, never hardcoded. **Reference platform (no-compromise ceiling): AMD Ryzen 9 9950X3D + NVIDIA RTX 5090 + 48 GB DDR5** → 200 actors + 500 projectiles + 1000 hazard pixels at sustained 60 Hz; 120 Hz mode hits 120 FPS sustained. Per-subsystem perf budgets in `summary.json.performance` (p50/p99/p999 microseconds per subsystem); CI regression gate (`m8a_perf_gate.sh`) blocks any ≥25% subsystem regression. Steam Deck and integrated-GPU machines are explicitly **anti-scoped** at M8A+ — they run M1-M8 content fine, but M9+ density is not guaranteed playable. The ceiling matters more than the floor for a game this physics-heavy. |

---

## Performance, Determinism, and Hardware Targets

> [!important]
> M8A is the milestone that locks Corefall's performance and determinism contract. It refactors the M1-M8 single-threaded `RwLock<EngineMutable>` foundation into a parallel-deterministic Bevy ECS scheduler, ships GPU cosmetic offload, locks the network protocol, and retrofits every M1-M6 subsystem to the new contract with byte-identical replay enforcement. Spec: [`specs/active/M8A.md`](specs/active/M8A.md).

### Reference platforms

| Role | Hardware | Why this tier exists |
|---|---|---|
| **Client (single-player + LAN host + competitive)** | AMD Ryzen 9 **9950X3D** (16 cores / 32 threads, 3D V-cache) · NVIDIA **RTX 5090** (32 GB VRAM) · 48 GB DDR5 · NVMe SSD · Windows 11 24H2 OR Linux 6.x | The no-compromise ceiling. 200 actors + 500 projectiles + 1000 hazard pixels at sustained 60 Hz; 120 Hz mode hits 120 FPS sustained. Cosmetic particle counts in the tens of thousands without sim cost. |
| **Server — workstation x86_64** | Ryzen 9 9950X3D OR Threadripper 7000 · NVIDIA RTX 5090 / 4090 · 64+ GB DDR5 · 1+ Gbps NIC | Dedicated public servers, large LAN tournaments, server-side replay rendering. GPU available for cosmetic stress tests + replay rendering. |
| **Server — Apple Silicon** | **Mac Studio / Mac mini M4 Pro** (10P+4E cores, 273 GB/s memory bandwidth) OR **M5 Pro / M5 Max** | Indie-host shards, persistent-world nodes, datacenter Apple racks. High single-thread perf (matches 9950X3D at 50-70% of perf-per-thread); unified memory; low TDP. No GPU determinism risk because GPU is excluded from the determinism island. |
| **Server — Linux VPS** | 16+ vCPU Linux x86_64 (Hetzner AX102 / OVH Game / AWS c7i.4xlarge) · no GPU required · 32+ GB RAM · 10 Gbps NIC | Cloud-hosted public servers, MMO shard backbone. `cf-headless serve` compiles with the `cf-render-2d/stub` feature for zero-GPU images. Same deterministic sim core as workstation. |
...
**Every server tier passes the same cross-OS determinism CI gate.** Linux x86_64 + macOS aarch64 + Windows x86_64 produce byte-identical final checksums on the same scenario.ron + seed + cfctl inputs. There is no "second-class" server hardware.

### Performance contract

| Subsystem | p99 budget (per tick, 16.6 ms total) | Notes |
|---|---:|---|
...
| Headroom | ≥ 4.0 ms | rollback resimulation buffer (6-frame rollback at p99 ≤ 2.5 ms / frame) |
| **Total budget** | **≤ 15.5 ms** | under the 16.6 ms 60 Hz wall-clock |

Reference-platform measurements are captured in every run bundle's `summary.json.performance` with p50/p99/p999 microseconds per subsystem. CI gate `m8a_perf_gate.sh` blocks merges on any ≥25% subsystem regression vs the locked baseline.

### Determinism contract (the hard rules)

CI gates enforce each:

1. **No `f64` in sim crates.** Use `f32` only. f32 has stable cross-platform bit-exact behavior under IEEE 754; f64's hardware-acceleration paths diverge across vendors.
2. **No `thread_rng()` in sim crates.** Every RNG call uses the engine's seeded RNG. Pre-roll into a `Vec<u64>` of length N BEFORE entering a `par_iter` block; workers index by stable entity id.
3. **No `HashMap` with default `RandomState` hasher in sim state.** Use `BTreeMap` (deterministic iteration) OR `FxHashMap` (fast, fixed seed — only safe when iteration order doesn't cross the checksum boundary).
4. **No FMA / wide AVX-512 intrinsics that vary by hardware.** Use the stable f32 code path.
5. **No `Instant::now()` / `SystemTime::now()` in sim code.** Use the engine's `Clock::tick().0` for any time-like value.
...
The combination of these rules is what allows a 9950X3D Windows client, an Apple M4 Pro macOS server, and a Hetzner Linux x86_64 VPS to produce byte-identical event streams on the same inputs.

### Networking

| Mode | When it ships | What it does |
|---|---|---|
| **`cf-headless serve` — dedicated authoritative server** | M8A | Same deterministic sim core as the client; no renderer; same run-bundle envelope. Server bundle is the lobby's source of truth for replay + dispute resolution + cross-client divergence detection. |
| **LAN lockstep** | M8A | Clients send only inputs; server broadcasts merged input set per tick; all clients sim-step locally with identical inputs. Byte-identical output enforced by the determinism contract. 8 clients on the same LAN see the same world. |
| **Internet rollback** | M8A | Client predicts forward, server confirms, client rolls back on misprediction + resimulates forward (6-frame budget at p99 ≤ 8 ms total). Resimulation re-uses the same deterministic sim core — no parallel rollback codepath. |
| **Semantic terrain events** | M8A | Every terrain mutation emits `terrain.chunk_mutated` with `cause` + `chunk_coords` + `bbox` + `delta_materials` + `post_state_checksum`. CCCP-era bitmap deltas are forbidden — semantic events are ~5x smaller wire-size and replayable independently of the bitmap. Fallback per-chunk snapshot when a client falls > 256 ticks behind. |
| **Snapshot envelope** | M8A | Powder-Toy-derived field shape: `tick + rng_state + chunks + actors + projectiles + mission` is the minimum byte set for byte-identical restore. Snapshot cadence: every 64 ticks (configurable via cfctl `srv.set_cvar net.snapshot.cadence_ticks <n>`). |

Source-level prior art audit + the multiplayer architecture decision matrix live in the research vault under `cortext_command_vault/engine/network-terrain-replication-lifecycle.md` + `cortext_command_vault/systems/replay-determinism-and-run-evidence.md`. The hybrid model adopted at M8A (server-authoritative-entity + semantic-terrain-events + LAN lockstep + internet rollback) is the vault's recommendation after auditing CCCP (RakNet bitmap deltas — rejected as anti-pattern), Powder Toy (snapshot field contract — adopted), OpenSoldat (GameNetworkingSockets / modernization patterns — partly adopted), OpenLieroX (`C2S_CARVE` protocol shape — adopted), Photon Quantum (replay envelope — adopted), Gaffer On Games (lockstep + snapshot interp + rollback — adopted), Factorio (snapshot-read / commit-serial — adopted as canonical parallel pattern), and Noita (chunk wake/sleep — adopted via M3's `active_region` flag).
...
| Networking | [quinn](https://github.com/quinn-rs/quinn) (QUIC over UDP) — locked at M8A per DR-052. Reliable streams for the canonical event log; unreliable datagrams for low-latency inputs + snapshot deltas. WebSocket fallback for future browser spectator clients. Wire protocol `prototype-net-frame.v0.1` (additive-only). |
| Server hardware tiers | (1) Workstation x86_64 + NVIDIA RTX 5090 + 9950X3D, (2) Apple Silicon Mac mini M4 Pro / M5+ (server runs without GPU), (3) Linux VPS (Hetzner / OVH / AWS c7i), (4) Apple Mini Lab cluster (3-5 nodes, 10 GbE) for persistent-MMO mesh dev. All four pass the same cross-OS determinism CI gate. |
...
├── cf-net                    # M8A     server + client + snapshot + transport (QUIC via quinn) + protocol v0.1 + rollback netcode
...
*Parallel-deterministic sim. Byte-identical replay across Linux, macOS, and Windows. 200-actor scenes at 60 Hz on the reference platform. Server runs on workstation x86_64, Apple Silicon, or a headless Linux VPS — same binary, same deterministic core.*
