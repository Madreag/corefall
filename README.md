...
[![Specs](https://img.shields.io/badge/active%20specs-31%20%28M2.2A..M12%29-blueviolet?style=flat-square)](specs/active/)
...
| **PvE Survival mode + Inter-Planet Transport** | Terraria/Stationeers/Minecraft hybrid solo + 2-8 player coop. **3 launch survival worlds** (Earth + Mars + Mimas; 9 more post-launch unlock). Procedurally-generated terrain + ore deposits + hazard zones + AI raider settlements per world. **3 inter-planet transport modes**: dropship (T1) + multi-stage rocket (T2) + paired teleporters (T3). Orbital stations + asteroid mining colonies. 5 PvE endgame bosses (Hollow King / Frozen Heart / Crimson Tide / Eclipse Walker / Last Star). 12 dynamic world events. Persistent multi-session base + worlds + veterans + AI commander rivalry. **Owned by M11.5.** |
| **Configurable everything — 7-tier settings hierarchy** | Every dial is a settings key with typed schema + default + range + description + owner. 7-tier hierarchy (engine → system → profile → scenario → server → CLI → runtime) with key locking. Server admin's `server.ron` controls 200+ tunables (anti-cheat, mods, AI, power, damage, crafting, etc.). Player's `profile.ron` for personal prefs. 20+ admin CLI commands (kick / ban / save / restart / hot-load / lock / unlock / mod / broadcast). Schema migration on bump. Settings included in replay determinism contract. **Owned by M9.10.** |
...
- **31 active milestone specs** in [`specs/active/M2.2A..M12.md`](specs/active/) — the executable implementation contracts for the gameplay spine. Each is read-by-implementing-agent-only per AGENTS.md.
