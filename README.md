...
**Releases:** every BP closure publishes a tagged cross-platform release under [github.com/Madreag/corefall/releases](https://github.com/Madreag/corefall/releases) per the **T-RELEASE** side track (Linux + Windows + macOS x86_64 / aarch64 archives + SHA256SUMS + run-bundle exemplar + summary_grid hero). Versioning uses SemVer prerelease channels so external observers can read quality from the tag without consulting the BP table: `v0.<N>.0-prealpha` (BP0..BP3, engine + first fun slices), `v0.<N>.0-alpha` (BP4..BP6, full collision + atmospherics + AI combat), `v0.<N>.0-beta` (BP7..BP9, mission director + creator alpha + server/LAN), `v0.<N>.0-rc` (BP10..BP11, online + public systems beta), `v1.0.0` (BP12 launch GA — pre-release flag dropped). Boundaries are enforced by `generate_release_notes.py`. Legacy `v0.<N>.0-bp<N>` tags are still accepted for backward compat. **Current release debt:** the retroactive `v0.1.0-prealpha` (BP1) and `v0.2.0-prealpha` (BP2) tags have not been published yet; publish both before BP3 closes.<br>
