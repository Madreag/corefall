...
├── cf-chassis              # (real)  3 archetypes + 15-zone body graph + layered armor + module state machine + pilot binding + eject + salvage
...
**BP2 closure recap (the most recent BP to close):** Chunked deformable terrain (M2) replaced the M1.5 soft-breach strip; M2.5 fun-proof scenario shows the player surviving a 60-90s reactor defense as terrain is dug + debris fields form; cf-headless re-runs any run bundle's `events.jsonl` deterministically (M3A). Engineering closed via PR [#11](https://github.com/Madreag/corefall/pull/11) (BP2 milestone code) + PR [#12](https://github.com/Madreag/corefall/pull/12) (source-truthful run bundles + AI-Agent Self-Test contract + LLM-graded test verdicts + per-BP test suite). PR [#13](https://github.com/Madreag/corefall/pull/13) was planning-spine migration (docs only, not engineering closure). PR [#14](https://github.com/Madreag/corefall/pull/14) was SemVer prerelease channel versioning (release infrastructure, not BP2 gameplay).
...
| BP2 | **M2.5 — Micro Reactor Defense** | ✅ **Closed** ([PR #11](https://github.com/Madreag/corefall/pull/11) merged) | 60-90s defend-the-reactor scenario; chunked terrain-driven win/loss; cfctl-scripted; same Acceptance + Contract Integrity gates as M1.5. |
...
| BP3 | **M4A — Readability + ACC-A Floor** | ✅ **Closed** ([PR #27](https://github.com/Madreag/corefall/pull/27) merged) | Silhouette HUD, module strip, movement/stance readability, banner stack, captions surface, tool-validity line, accessibility floor (200% scale + high contrast + reduced_* + focus traversal, hold-to-confirm, validated remap table, captions). DR-003 + DR-012 closed. **M4B comic-noir polish (mission cards, stylized banners) deferred to BP7 per Roadmap V2 split; M4-P00 is [~] partial.** |
